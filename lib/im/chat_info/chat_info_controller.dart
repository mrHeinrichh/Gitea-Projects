...
class ChatInfoController extends GetxController
  ...
  Rxn<String> signatureString = Rxn<String>();
  ...
  ChatInfoController.desktop(this.uid);
  ...
  generateEncryptionQR() async{
    if (isEncrypted.value) {
      // signatureBase64.value = objectMgr.encryptionMgr.signMessage(chat.value!.chat_id);
      signatureBase64.value = await objectMgr.encryptionMgr.getSignedKey(chat.value!);
      // signatureString.value = objectMgr.encryptionMgr.getSignedText(signatureBase64.value!);
      // signatureString.value = objectMgr.encryptionMgr.getSignatureText(signatureBase64.value!);
      // var b = objectMgr.encryptionMgr.verifySignature(chat.value!.chat_id, signatureBase64.value!);
    }
  }

  Future<void> _updateGroupExit() async {
    List result = await checkGroupExist();
  ...
  void addFriend(context) async {
    if (ableAddFriend.value && !isSendingFriendReq) {
      showConfirmModal(user.value!, context);
    }
  }
  ...
  void withdrawRequest(context) async {
    final success = await objectMgr.userMgr.withdrawFriend(user.value!);
    if (success) {
      imBottomToast(
        context,
        title: localized(successWithdrawReq, params: [user.value!.nickname]),
  ...
  showConfirmModal(User user, ctx) async {
    ...
    if (errorCount > 0) {
      ...
      if (chats.length > 1) {
        imBottomToast(
          ...
          !Get.currentRoute.contains("${chats.first.id}")) {
        imBottomToast(
          navigatorKey.currentContext!,
          title: localized(messageShareSuccess, params: [name]),
          icon: ImBottomNotifType.success,
