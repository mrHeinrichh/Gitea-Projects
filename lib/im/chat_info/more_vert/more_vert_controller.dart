...
/*    AutoDeleteMessageModel(
  ...
  Future<void> onEncryptionToggle() async {
    Chat c = chatInfoController?.chat.value ?? groupChatInfoController!.chat.value!; //不是单聊就是群聊
    List<int> users = [];
    if (c.isSingle) {
      users = [objectMgr.userMgr.mainUser.uid,c.friend_id];
    } else {
      users.assignAll(groupChatInfoController!.groupMemberListData.map((element) => element.uid).toList());
    }
  ...
  onShowInviteGroupDialog() {
    ...
    showModalBottomSheet(
      ...
      builder: (BuildContext context) {
        ...
        Get.delete<GroupInviteLinkController>());
  }
  ...
  Future<void> _showScreenNotificationPopup() async {
    ...
    showCustomBottomAlertDialog(
      groupChatInfoController != null
          ? groupChatInfoController!.context
          : chatInfoController!.context,
      subtitle: localized(isEnabled
          ? turnOffScreenshotDetail
          : turnOnScreenshotDetail),
  ...
  Future<void> _showDeleteChatHistorySheet() async {
    showCustomBottomAlertDialog(
      ...
      onConfirmListener: () {
        Get.until((route) => Get.currentRoute == RouteName.home);
        String str = groupChatInfoController != null
            ? localized(theGroupAlreadyDeleted)
            : localized(deleteMyChatRecord);
      ...
      onConfirmListener: () {
        if (groupChatInfoController!.isOwner.value) {
          ...
          imBottomToast(
            ...
            duration: 5,
            withCancel: true,
            timerFunction: groupChatInfoController!.notOwnerLeaveGroup,
            undoFunction: () => BotToast.removeAll(BotToast.textKey),
          );
        }
      },
