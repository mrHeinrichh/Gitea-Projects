...
class CustomCupertinoDatePicker extends StatefulWidget {
  ...
  CustomCupertinoDatePicker({
    ...
    assert(
    (mode != CustomCupertinoDatePickerMode.date && mode != CustomCupertinoDatePickerMode.monthYear) || (minimumYear >= 1 && this.initialDateTime.year >= minimumYear),
    'initial year is not greater than minimum year, or minimum year is not positive',
    );
    assert(
    (mode != CustomCupertinoDatePickerMode.date && mode != CustomCupertinoDatePickerMode.monthYear) || maximumYear == null || this.initialDateTime.year <= maximumYear!,
    'initial year is not smaller than maximum year',
    );
    assert(
    (mode != CustomCupertinoDatePickerMode.date && mode != CustomCupertinoDatePickerMode.monthYear) || minimumDate == null || !minimumDate!.isAfter(this.initialDateTime),
    'initial date ${this.initialDateTime} is not greater than or equal to minimumDate $minimumDate',
    );
    assert(
    (mode != CustomCupertinoDatePickerMode.date && mode != CustomCupertinoDatePickerMode.monthYear) || maximumDate == null || !maximumDate!.isBefore(this.initialDateTime),
  ...
  double _getEstimatedColumnWidth(_PickerColumnType columnType) {
    if (estimatedColumnWidths[columnType.index] == null) {
      estimatedColumnWidths[columnType.index] =
          CustomCupertinoDatePicker._getColumnWidth(columnType, localizations, context, widget.showDayOfWeek);
    }
  ...
  void _pickerDidStopScrolling() {
    ...
    if(selectedDate.isBefore(DateTime.now())){
      SchedulerBinding.instance.addPostFrameCallback((Duration timestamp) {
        ...
        _animateColumnControllerToItem(hourController, initialDateTime.hour);
        _animateColumnControllerToItem(minuteController, initialDateTime.minute ~/ widget.minuteInterval);
      });
    }
  }
...
class _CustomCupertinoDatePickerDateState extends State<CustomCupertinoDatePicker> {
  ...
  void _refreshEstimatedColumnWidths() {
    estimatedColumnWidths[_PickerColumnType.dayOfMonth.index] = CustomCupertinoDatePicker._getColumnWidth(_PickerColumnType.dayOfMonth, localizations, context, widget.showDayOfWeek);
    estimatedColumnWidths[_PickerColumnType.month.index] = CustomCupertinoDatePicker._getColumnWidth(_PickerColumnType.month, localizations, context, widget.showDayOfWeek);
    estimatedColumnWidths[_PickerColumnType.year.index] = CustomCupertinoDatePicker._getColumnWidth(_PickerColumnType.year, localizations, context, widget.showDayOfWeek);
  }
  ...
  String getMonthStr(int month) {
    if(localizations is CupertinoLocalizationZh){
      return "$month月";
    }
    return localizations.datePickerMonth(month);
  }
}
...
class _CustomCupertinoDatePickerMonthYearState extends State<CustomCupertinoDatePicker> {
  ...
  void _refreshEstimatedColumnWidths() {
    estimatedColumnWidths[_PickerColumnType.month.index] = CustomCupertinoDatePicker._getColumnWidth(_PickerColumnType.month, localizations, context, false);
    estimatedColumnWidths[_PickerColumnType.year.index] = CustomCupertinoDatePicker._getColumnWidth(_PickerColumnType.year, localizations, context, false);
  }
