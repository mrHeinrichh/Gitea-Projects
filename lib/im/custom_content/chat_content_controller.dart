...
class ChatContentController extends GetxController {
  ...
  reachBottom() async {
    ...
    if (chatController.mentionChatIdxList.isNotEmpty) {
      chatController.mentionChatIdxList.clear();
    }
    if (objectMgr.chatMgr.mentionMessageMap[chatController.chat.id] != null) {
      objectMgr.chatMgr.mentionMessageMap[chatController.chat.id]?.clear();
    }
    if (chatController.chat.unread_count > 0) {
      int unreadCount = chatController.chat.unread_count;
      chatController.chat.unread_count = 0;
      objectMgr.chatMgr.totalUnreadCount.value -= unreadCount;
      if (objectMgr.chatMgr.totalUnreadCount.value < 0) {
        objectMgr.chatMgr.totalUnreadCount.value = 0;
      }
      if (Get.isRegistered<ChatItemController>(
        tag: 'chat_item_${chat!.id.toString()}',
      )) {
        Get.find<ChatItemController>(tag: 'chat_item_${chat!.id.toString()}')
            .unreadCount
            .value = chatController.chat.unread_count;
      }
  ...
  scrollToBottomMessage({int fromIdx = -1}) async {
    ...
    if (messageList.isNotEmpty) {
      ...
      for (var message in chatController.previousMessageList) {
      ...
      objectMgr.chatMgr.updateChatAfterSetRead(
        ...
        chatController.chat.read_chat_msg_idx,
      );
  ...
  updateMention(Message msg) {
    ...
    if (objectMgr.chatMgr.mentionMessageMap[chatController.chat.chat_id] !=
        null) {
      objectMgr.chatMgr.mentionMessageMap[chatController.chat.chat_id]!
          .removeWhere((key, value) => value.chat_idx <= lastVisibleUnreadIdx);
    }
  }
