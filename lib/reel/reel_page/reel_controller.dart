...
class ReelController extends GetxController with GetTickerProviderStateMixin {
  ...
  Rxn<ReelMyProfileController> profileController =
  Rxn<ReelMyProfileController>();
  ...
  void onInit() {
    ...
    Future.delayed(const Duration(milliseconds: 250), () {
      ...
      profileController.value =
          reelNavigationMgr.addMyProfile(); //我的页面预下载数据（以达到秒切效果）
  ...
  void onBottomTap(int index, {
    int nextSelected = 0,
    bool keepHomeCurrent = false,
  }) {
    if (index == 1) {
      doUploadPage();
    } else {
      if ((tabController!.index != 0 && index == 0) ||
          (index == 0 && keepHomeCurrent == true)) {
        onReturnNavigation();
      } else {
        if (index != 0) {
          onNavigation();

          profileController.value?.updateController();
          profileController.value?.selectedTab = nextSelected;
          if (profileController.value?.tabController != null) {
            profileController.value?.tabController?.animateTo(nextSelected);
          }
        } else {
          onTapHomePage();
        }
  ...
  void precachePostAsset() {
    ...
    if (!checkVideoCacheExist(currentPage)) {
      ...
      preparePlayer(
          currentPage, source, reel.thumbnail.value!, reel.gausPath.value,
          width,
          height); //第一道视频需马上显示
    }
    ...
    if (currentPage + cacheSegmentCount < postList.length &&
      ...
      if (isEnteringScreen.value) {
        ...
        _enteringScreenFunctions.add(() {
          preparePlayer(
              currentPage + 1, source, reel.thumbnail.value!,
              reel.gausPath.value, width, height);
        });
      } else {
        preparePlayer(
            currentPage + 1, source, reel.thumbnail.value!, reel.gausPath.value,
            width, height);
      }
    }
  }
  ...
  ) getVideoWidthAndHeight(int videoWidth,
      int videoHeight,
