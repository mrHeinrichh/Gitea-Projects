    ...
    if (errorCount > 0 && index != -1) {
      if (dataList.length > 1) {
        await showModalBottomSheet(
          ...
          builder: (BuildContext context) {
            return CustomConfirmationPopup(
              title: localized(someOfSelectedItemCannotBeDeleted),
              confirmButtonColor: colorRed,
              cancelButtonColor: themeColor,
              confirmButtonText: localized(reelDeleteConfirm),
              cancelButtonText: localized(buttonCancel),
              confirmCallback: () {
                dataList.removeWhere(
                    (element) => unableDeleteList.contains(element));
        ...
        await showModalBottomSheet(
          ...
          builder: (BuildContext context) {
            return CustomConfirmationPopup(
              title: localized(jumpToOriginalFavourite),
