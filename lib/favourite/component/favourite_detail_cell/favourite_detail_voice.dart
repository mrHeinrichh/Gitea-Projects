...
class _FavouriteDetailVoiceState extends State<FavouriteDetailVoice> with WidgetsBindingObserver {
  ...
  void getVoicePath() async {
    ...
    if (voiceFilePath == null || voiceFilePath!.isEmpty) {
      voiceFilePath = await cacheMediaMgr.downloadMedia(widget.data.url) ?? '';
    }
  ...
  Future<void> _onPlayer() async {
    ...
    if (currentPlayingFileName != playbackKey) {
      ...
      if (voiceFilePath == null || voiceFilePath!.isEmpty) {
        voiceFilePath =
            await cacheMediaMgr.downloadMedia(widget.data.url) ?? '';
      }
      if (voiceFilePath == null || voiceFilePath!.isEmpty) {
