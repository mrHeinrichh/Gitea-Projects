...
class BlockListController extends GetxController {
  ...
  getList() async {
    ...
    if (res["total_count"] > 0) {
      userList.addAll((res['users'] as List<dynamic>).map((item) => User.fromJson(item)).toList());
    }
  }
  ...
  void updateUser(List<User> unblockUserList, Map<String, dynamic>? relationships) {
    for (User user in unblockUserList){
      if (relationships!.isNotEmpty && relationships.keys.contains(user.accountId)){
