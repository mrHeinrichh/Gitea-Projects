...
class ChatHelp {
  ...
  static int getSplitPosition(
    ...
    for (int i = 0; i < sublist.length; i++) {
      ...
      if (message.read_num == 1) continue;
      ...
      if (message.typ == messageTypeImage ||
        ...
        count++;
          ...
          message.typ == messageTypeLink) {
        MessageText msgText = message.decodeContent(cl: MessageText.creator);
        lines = lines +
            getTextMessageBubbleLines(
              msgText.text,
              jxDimension.groupTextMeMaxWidth() - 26,
              setWidth(showPinned, message.edit_time > 0),
              message,
            );
  ...
  static bool hasEncryptedFlag(int flag) {
    return flag & ChatEncryptionFlag.encrypted.value == ChatEncryptionFlag.encrypted.value;
  }
}
