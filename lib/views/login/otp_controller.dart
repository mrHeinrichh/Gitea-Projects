  ...
  void accountChecking() async {
    ...
    if (fromView.value == OtpPageType.login.page) {
      ...
      try {
        ...
        /// 加密
        //objectMgr.encryptionMgr.getCipherKey();
    ...
    } else if (fromView.value == OtpPageType.encryption.page) {
      verifyOtpCode(
        OtpPageType.encryption.page,
        OtpPageType.encryption.type,
      );
  ...
  Future<void> verifyOtpCode(String fromView, int otpType) async {
    try {
      ...
      if (data != null && data.token != "") {
        ...
        } else if (fromView == OtpPageType.encryption.page){
          Get.close(1);
