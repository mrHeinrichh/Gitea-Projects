...
Future<ResponseData> hide(int user_id, int chat_id, int isHide) async {
  const apiPath = '/im/chat/set_hide';
...
Future<ResponseData> unpin_all(int chatId, List<int> messageIds) async {
  ...
  final res = await CustomRequest.doPost("/im/chat/unpin/all", data: data);
  return res;
}
...
String? encodeContent(
  ...
  try {
    if (isGroup) {
      ...
      User? user = objectMgr.userMgr.getUserById(friendId);
      if (user == null || user.publicKey == "") {
        return null;
      }

      String randKey = EncryptionMgr.getRandomString(16);
      String firendKey = RSAEncryption.encrypt(randKey, user.publicKey);
      String selfKey = RSAEncryption.encrypt(randKey,objectMgr.encryptionMgr.encryptionPublicKey);
      AesEncryption aes = AesEncryption(randKey);
      String newContent = aes.encrypt(content);
      var data = {
        'sk': selfKey,
        'fk': firendKey,
        'data': newContent,
...
Future<ResponseData> send(
  ...
  if(refType == 1 && chatKey != "" && type != messageTypeRespSignChat){
    data["content"] = encodeContent(content, isGroup, friendId, chatKey);
    if(isGroup){
      data['ref_typ'] = 1;
    }else{
      data['ref_typ'] = 2;
