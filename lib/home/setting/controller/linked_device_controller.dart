...
class LinkedDeviceController extends GetxController {
  ...
  getSettingService() async {
    isSingleDevice.value = objectMgr.localStorageMgr.read("only_allowed_single_device") ?? 0;
    ...
    objectMgr.localStorageMgr.write("only_allowed_single_device", isSingleDevice.value);
  }
  ...
  void updateSingleDevice(bool value) {
    if (value){
      ...
      int? mobileCount = otherDeviceList.where((item) => item.platform == 'app').length;
      int? desktopCount = otherDeviceList.where((item) => item.platform == 'desktop').length;
        ...
        showModalBottomSheet(
          ...
          builder: (BuildContext context) {
            return CustomConfirmationPopup(
              withHeader: true,
              title: localized(multiDeviceLogin),
              confirmButtonText: localized(turnOffMultiDeviceLogin),
  ...
  Future<void> setSingleDevice() async {
    final success = await SettingServices().updateSingleDevicePrivacy("only_allowed_single_device", isSingleDevice.value);
    if (success) {
      objectMgr.localStorageMgr.write("only_allowed_single_device", isSingleDevice.value);
      getDeviceList();
    }
