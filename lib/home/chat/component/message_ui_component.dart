...
class MessageUIComponent extends MessageUIBase<ChatListController> {
  ...
  Widget contentBuilder() {
    final String text =
        getMessageText(message, searchText: searchText?.toLowerCase());
  ...
  List<InlineSpan> getSpanList(
      String text, String? regexText, Color textColor) {
  ...
  String getMessageText(Message message, {String? searchText = ''}) {
    ...
    if (translationModel != null && translationModel.showTranslation) {
      ...
      if (translationModel.visualType == TranslationModel.showTranslationOnly) {
        return text;
      }
    }

    if (searchText != null && searchText.isNotEmpty && text.isNotEmpty) {
      if (text.toLowerCase().contains(searchText)) return text;
    }
