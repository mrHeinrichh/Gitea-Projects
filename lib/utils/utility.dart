...
Future<bool> checkPermission() async {
  var a = await Permissions.request([Permission.camera]);
  if (!a) {
    return false;
  }
...
Future<io.File> copyImageFile(io.File imageFile) async {
  ...
  final newFile = await io.File(
    '${directory.path}/${md5.convert(convert.utf8.encode(DateTime.now().toString()))}.jpg',
  ).create();
...
Future<FilePickerResult?> renameFiles(FilePickerResult? result) async {
  ...
  for (var file in result.files) {
    ...
    try {
      ...
      io.File renamedFile = await originalFile.rename(newPath);
...
Future<void> openFileDocument(String url, String fileName) async {
  ...
  cacheMediaMgr
      .downloadMedia(url, timeout: const Duration(seconds: 3000))
...
void showLoadingPopup(BuildContext context, DialogType type, String message) {
  ...
  showDialog(
    ...
    builder: (BuildContext context) {
      return Dialog(
        ...
        backgroundColor: colorTextPrimary.withOpacity(0.82),
