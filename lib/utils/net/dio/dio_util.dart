...
class DioUtil {
  ...
  Future<int> _putUriByLocalHost(
    ...
    sendData() async {
      const len = 1000;
      final total = (intList.length / len).ceil();
      for (var i = 0; i < total; i += 1) {
        await Future.delayed(
          Duration.zero,
        ).then((_) {
          bytesUploaded += len;
          onSendProgress?.call(len, contentLength);
          pdebug(
              'UploadedByLocalHost $bytesUploaded of $contentLength bytes (${(bytesUploaded ~/ contentLength * 100).toStringAsFixed(2)}%)');
          request.sink.add(
            intList.sublist(i * len, min((i + 1) * len, intList.length)),
          );
        });
      }
   ...
   * 下载文件
   */
  Future<Response> downloadUriFile(
    Uri uri,
    String savePath, {
