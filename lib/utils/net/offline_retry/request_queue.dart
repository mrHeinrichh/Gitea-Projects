...
class RequestQueue {
  ...
  Future<void> addRetry(
    ...
    addRetryWithExistingRetry(retry, completer, requestData);
  }

  Future<void> addRetryWithExistingRetry(
      Retry retry, Completer<ResponseData> completer, RequestData requestData,
    ...
    if (!_queue.any((task) => task.retry.uid == retry.uid)) {
      if (retry.replace != RetryReplace.NO_REPLACE) {
        ...
        var existingTask = _queue.firstWhereOrNull(
          (task) => task.retry.endPoint == retry.endPoint,
        );
  ...
  Future<ResponseData> _retryRequest(
      _RequestTask requestTask, RequestData requestData) async {
    ...
    while (DateTime.now().isBefore(endTime)) {
      ...
      try {
        ...
        if (requestTask.retry.apiType == CustomRequest.methodTypeGet) {
          ...
          return await CustomRequest.doPost(
            requestTask.retry.endPoint,
            data: requestData.data,
            attempts: requestData.attempts,
            refreshAttempts: requestData.refreshAttempts,
  ...
  Future<void> restoreQueue() async {
    ...
    for (var retry in allRetry) {
      ...
      addRetryWithExistingRetry(
          retry, completer, RequestData.fromJson(jsonDecode(retry.requestData)),
          isReload: true);
    }
  }
