    ...
    return await _msgProcessLock.synchronized(() async {
      try {
        ...
        if (isReSend && chat != null) {
          var rep =
              await _checkReSend(chat, sendTime, message, firstSendTime, 1);
          if (rep != null) {
            return rep;
          }
        }
  ...
  Future<bool> _doMediaGausProcess(Message message) async {
    if (message.isMediaType) {
      ...
      for (final assetInfo in assetList) {
        // 检查url的文件是否存在
        String? localPath = downloadMgr.checkLocalFile(assetInfo['url']);
        // 不存在就下载
...
abstract mixin class ChatSend {
  ...
  Future<ResponseData> sendText(
    ...
    dict['text'] = content;
    var typ = isLink ? messageTypeLink : messageTypeText;
    if (reply != null) {
      dict['reply'] = reply;
