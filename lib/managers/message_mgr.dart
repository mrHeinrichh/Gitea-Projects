...
class MessageMgr extends EventDispatcher {
  ...
  final Map<String, int> _msgReqSendTimeMap = {};
  ...
  static decodeMessage(Message message, FetchingMsgChat chat, int myUid) {
    if (message.ref_typ == 1 && chat.chatKey != "") {
      try {
        //pdebug("--decodeMessage---chat.chatKey:${chat.chatKey}");
        AesEncryption aes = AesEncryption(chat.chatKey);
    ...
    } else if (message.ref_typ == 2 && chat.chatKey != "") {
      try {
        var data = json.decode(message.content);
        //pdebug("--decodeMessage---content:${data["data"]}");
        if (data["sk"] != null && data["fk"] != null && data["data"] != null) {
            if (message.send_id == myUid) {
              if(chat.selfEncodeKey != data["sk"] && data["sk"] != ""){
                String selfKey = RSAEncryption.decrypt(data["sk"], chat.chatKey);
                chat.selfKey = selfKey;
                chat.selfEncodeKey = data["sk"];
              }
              //pdebug("--decodeMessage---selfKey:${chat.selfKey}");
              AesEncryption aes = AesEncryption(chat.selfKey);
              message.content = aes.decrypt(data["data"]);
              message.ref_typ = 0;
            }else {
              if(chat.friendEncodeKey != data["fk"] && data["fk"] != ""){
                String friendKey = RSAEncryption.decrypt(data["fk"], chat.chatKey);
                chat.friendKey = friendKey;
                chat.friendEncodeKey = data["fk"];
              }
              //pdebug("--decodeMessage---friendKey:${chat.friendKey}");
              AesEncryption aes = AesEncryption(chat.friendKey);
              message.content = aes.decrypt(data["data"]);
              message.ref_typ = 0;
            }
        }
  ...
  Future<void> init(int uid, String token) async {
    try {
      await initLock.synchronized(() async {
        ...
        for (int i = 0; i < 3; i++) {
          try {
            ...
            isolate = await Isolate.spawn(msgLoadIsolate.handle, [
              ...
              decryptChatReceivePort.sendPort,
            ]);
            ...
            pdebug(e);
            pdebug(stackTrace);
            debugInfo.printErrorStack(e, stackTrace, toast: Config().isDebug);
          }
        ...
        _decryptChatSendPort = await decryptChatReceivePort.first;
  ...
  /// 变更子线程的日志开关，但实际子线程未处理传递的消息，该方法未起作用
  ...
  void handleMsgContent(UpdateBlockParser parser) {
    for (var item in parser.clientActions) {
      if (item.action == ACTION_HISTORY_MSG) {
        ...
        int? startTime = _msgReqSendTimeMap.remove(item.requestId);
        startTime ??= 0;

        // 通过性能日志，记录从发送消息拉取ws请求到接收到所有消息的耗时
        logMgr.metricsMgr.addMetrics(
          Metrics(
...
class MsgLoadIsolate {
  ...
  void handle(List<dynamic> args) async {
    try {
      ...
      SendPort decryptChatsSendPort = args[14];
  ...
  editMessageProcess(Message msg, List<Message> list) async {
    ...
    if (editMsg != null) {
      ...
      if (messageEdit.atUser != '[]' && messageEdit.atUser.isNotEmpty) {
        editMsg.atUser = jsonDecode(messageEdit.atUser)
            .map<MentionModel>((e) => MentionModel.fromJson(e))
            .toList();
      }
      list.add(editMsg);
  ...
  /// 保存消息
  Future<bool> _saveMessage(FetchingMsgChat chat, List<Message> messages, {bool isCallBack = true}) async {
    if (messages.isEmpty) {
      return true;
    }
  ...
  _decryptChats(List<FetchingMsgChat> chats) async {
    ...
    for (FetchingMsgChat chat in chats) {
        ...
        if (chatAndMessage != null && chat.chatKey != "") {
  ...
  _decryptChat(FetchingMsgChat chat) async {
    ...
    for (String tableName in messageTables) {
      while (true) {
        ...
        List<Message> messages =result.map<Message>((e) => Message()..init(e)).toList();
      ...
      try {
        ...
        for (ChatAndMessage chatMessage in _msgCache.values) {
          ...
          if (fetchingMsgChat.lastPos != fetchingMsgChat.oldLastPos) {
            batch.update(
              "chat",
              {"last_pos": fetchingMsgChat.lastPos},
              where: 'id = ?',
              whereArgs: [fetchingMsgChat.chatId],
            );
