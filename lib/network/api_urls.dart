...
class _Util {
  ...
  _Util() {
    // pdebug('解密:${decrypt2Base64(_ApiUrlsHost)}');
  }
    ...
    for (var i = 0; i < 1; i++) {
      try {
        ...
        if (rep is String) {
          ...
          // pdebug('解析域名地址${url.toString()}');
          return jsonDecode(rep);
        }
      } catch (e) {
        pdebug('httpGetJson jsonDecode:$e');
      }
    }
    return null;
  }
  ...
  Future<dynamic> httpGet(Uri url) async {
    ...
    try {
      ...
      if (status != HttpStatus.ok) {
        pdebug('dartGet status:$status body:$responseBody');
        return null;
      }
      return responseBody;
    } catch (e) {
...
class ApiUrls extends _Util {
  ...
  Future<void> _saveToLocal() async {
    ...
    await setString('ApiUrls.dunKiwiStart$appVer', dunKiwiStart);
  }
  ...
  Future<bool> next(device, channel) async {
    pdebug("尝试取得新的地址: device:$device channel:$channel");
    // 如果对象还没有初始化初始化一下
    if (_apiUrlsMgr == null) {
    ...
    if (json == null) {
      ...
      pdebug('_apiUrlsMgr.tryPrevPos 取得新的位置 $pos');
      await _apiUrlsMgr!._saveToLocal();
      if (pos < 0) {
        pdebug('已经取不到地址了。。 重新开始了');
        //重新初始化地址
...
class _ApiInfoManager extends _Util {
  ...
  _debug() {
    ...
    pdebug('_ApiInfoManager._debug.tryPrevPos ${hosts.length} $ar');
  }
  ...
  Future<bool> _init(String str, int version) async {
    ...
    pdebug('_ApiInfoManager _init:$str, $version');
  ...
  Future<bool> init(k) async {
    ...
    if (_version == null || _version == 0) {
      pdebug('_ApiInfoManager 本地版本为空，初始化：{${Config().kiwiVersion}');
    ...
    final t = (await getString('_ApiInfoManager._pos')).split(',');
    for (var i = 0; i < t.length; i++) {
      _pos[i] = int.tryParse(t[i])!;
    }
