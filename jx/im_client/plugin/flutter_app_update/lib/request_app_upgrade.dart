...
Client getHttpClient1() {
  ...
  Client client = IOClient(ioClient);
  return client;
}
...
class RequestAppUpgrade {
  ...
  static Future<HttpResponseBean> send(String url,
      [String method = RequestAppUpgrade.methodTypeGet,
      dynamic data,
    ...
    try {
      ...
      //print("Request.send: $uri");
      HttpClientRequest? request;

      if (method == RequestAppUpgrade.methodTypePost) {
        request = await httpClient.postUrl(uri);
      } else if (method == RequestAppUpgrade.methodTypePut) {
        request = await httpClient.putUrl(uri);
      } else if (method == RequestAppUpgrade.methodTypeGet) {
      ...
      if ((method == RequestAppUpgrade.methodTypePost ||
              method == RequestAppUpgrade.methodTypePut) &&
          data != null) {
        var parts = [];
