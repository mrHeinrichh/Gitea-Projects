...
abstract class AssetPickerViewerBuilderDelegate<Asset, Path> {
  ...
  void selectAsset(Asset entity) {
    ...
    if (!isSelectedPreviewing) {
      // prevent duplicate photo sent
      if (selectedAssets != null && !selectedAssets!.contains(entity)) {
        selectedAssets?.add(entity);
        WidgetsBinding.instance.addPostFrameCallback((_) {
          previewingListController.animateTo(
              previewingListController.position.maxScrollExtent,
  ...
  Future<bool> onChangingSelected(
    ...
    // final File? data = await (asset as AssetEntity).originFile;
      ...
      child: Consumer<AssetPickerViewerProvider<AssetEntity>?>(
        builder: (_, AssetPickerViewerProvider<AssetEntity>? provider, __) {
          ...
          String buildText() {
            ...
            // }
            if (provider!.isSelectedNotEmpty) {
              return '(${provider.currentlySelectedAssets.length}/${selectorProvider!.maxAssets})${textDelegate.confirm}';
            }
            return textDelegate.confirm;
          }
    ...
    return Semantics(
      ...
      child: ExtendedImageGesturePageView.builder(
        ...
        onPageChanged: (int index) {
          ...
          // previewingListController.animateTo(
          //   previewingListController.position.maxScrollExtent + 10,
          //   curve: Curves.ease,
          //   duration: kThemeChangeDuration,
          // );
        },
