  ...
  Widget build(BuildContext context) {
    return Listener(
      ...
      child: AnimatedContainer(
        ...
        duration: Duration(milliseconds: isPressed ? 1 : 300),
        decoration: BoxDecoration(
          color: (isPressed && widget.withEffect == true)
              ? (widget.overlayColor ?? const Color(0x0f000000))
              : Colors.transparent,
...
class _OverlayEffectMenuState extends State<OverlayEffectMenu> {
  ...
  Widget build(BuildContext context) {
    return AnimatedContainer(
      ...
      duration: Duration(milliseconds: isPressed ? 1 : 300),
      decoration: BoxDecoration(
        color: ((isPressed || (widget.isHighLight ?? false)) &&
                widget.withEffect == true)
            ? (widget.overlayColor ?? const Color(0x0f000000))
            : Colors.transparent,
        borderRadius: widget.radius ?? BorderRadius.zero,
      ),
...
class _OpacityEffectState extends State<OpacityEffect> {
  ...
  Widget build(BuildContext context) {
    return Listener(
      ...
      behavior: HitTestBehavior.translucent,
      child: Opacity(
        opacity: isPressed ? widget.opacity : 1,
        child: widget.child,
      ),
    );
  }
        ...
        children: <Widget>[
          ...
          Positioned.fill(
            child: AnimatedContainer(
              ...
              decoration: BoxDecoration(
                color: (isPressed && widget.withEffect == true)
                    ? (widget.overlayColor ?? const Color(0x14000000))
                    : Colors.transparent,
                borderRadius: widget.radius ?? BorderRadius.zero,
              ),
            ),
