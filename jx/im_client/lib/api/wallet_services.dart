...
class WalletServices {
  final String _walletUrl = '/app/api/wallet';

  Future<WalletAssetsModel?> getUserAssets({
    String currency = 'USD',
  ...
  Future<RedPacketDetail> getRedPacket({
    ...
    try {
      final ResponseData res = await CustomRequest.doGet(url, data: params);
  ...
  Future<List<RedPacketDetail>> getRedPacketMultiple({
    ...
    try {
      final ResponseData res = await CustomRequest.doPost(url, data: params);
  ...
  Future<ResponseData> sendRedPacket({
    ...
    try {
      final ResponseData res =
          await CustomRequest.doPost(url, data: redPacketModel.toJson());
      if (res.success()) {
        return res;
      } else {
        throw AppException(res.code, res.message);
  ...
  Future<Map> validateAddress({
    ...
    try {
      ...
      if (address != '' || address.isNotEmpty) {
        showWarningToast(localized(walletAddressInvalid));
      }
      rethrow;
    }
  }
  ...
  Future<Map> getRedPacketConfig() async {
    ...
    try {
      final ResponseData res = await CustomRequest.doGet(url);
  ...
  //錢包轉帳
  Future<ResponseData> walletTransfer(
    int toUserID,
    String amount,
    String currencyType,
    String passcode,
  ...
  Future<ResponseData> getWalletAssets(Map<String, dynamic> map) async {
    ...
    ResponseData res = await CustomRequest.doGet(url, data: map);
    return res;
  }
  ...
  Future<ResponseData> postTransferChat(Map<String, dynamic> map) async {
    ...
    try {
      ResponseData res = await CustomRequest.doPost(url, data: map);
      return res;
    } on AppException catch (e) {
      return ResponseData(
        code: e.getPrefix(),
  ...
  ///收款付款说明需要的数据
  Future<ResponseData> getReceiveAndPayExpalinData() async {
    String url = '/payment/blockchain-config/get';
    try {
      ResponseData res = await CustomRequest.doPost(url, data: {});
      return res;
    } on AppException catch (e) {
      return ResponseData(
        code: e.getPrefix(),
  ...
  Future<ResponseData> getSettings() async {
    ...
    try {
      ResponseData res = await CustomRequest.doGet(url);
      return res;
    } on AppException catch (e) {
      return ResponseData(
        code: e.getPrefix(),
  ...
  ///获取当日交易额
  Future<ResponseData> postWalletTodayTotalSettings(
  ...
  Future<ResponseData> _updateBlockchainSettings({
    ...
    try {
      ResponseData res = await CustomRequest.doPost(url, data: data);
      return res;
    } on AppException catch (e) {
  ...
  ///更新二次校验限额
  Future<ResponseData> postDailyTransferUpdate(
  ...
  ///更新二次校验配置
  Future<ResponseData> postTwoFactorAuthSettingsUpdate(
    Map<String, dynamic> data,
  ) async {
