...
class QueueDownloadTaskMgr extends ScheduleTask {
  ...
  clear() async {
    for (QueueDownloadTask task in _taskList) {
...
class QueueDownloadTask implements Comparable<QueueDownloadTask> {
  ...
  final void Function(QueueDownloadTaskEnum status, String str, int startTime)?
      onComplete;
  ...
  QueueDownloadTask({
    ...
    updateTime = createTime ?? DateTime.now().millisecondsSinceEpoch;
  }
  ...
  Future<void> _executeInternal() async {
    try {
      ...
      if (completer.isCompleted) {
        onComplete?.call(status, toString(), createTime);
      }
    }
  }
  ...
  Future<QueueDownloadResultEnum> _runWithTimeout() {
    return Future.any([
      task(cancelToken, redirect, timeout).whenComplete(() => null),
