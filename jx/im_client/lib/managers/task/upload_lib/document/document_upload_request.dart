...
class DocumentUploadRequest extends UploadChunk {
  ...
  DocumentUploadRequest(super.sourceFilePath,
      {required super.cancelToken,
      super.onSendProgress,
      this.onFileCoverGenerated,
      super.storageType = StorageType.document}) {
  ...
  Future<DocumentUploadRequest> prepareHandle(
    ...
    if (request.sourceFilePath.toLowerCase().contains('.pdf')) {
      try {
        ...
        PdfPageImage img = await page.render(width: 480, height: 480);
        ...
        String path = "";
        if (byteData != null) {
          path = await downloadMgr.getTmpCachePath(
        ...
        request.onFileCoverGenerated?.call(path, path.isEmpty, type);
        doc.dispose();
      } catch (e) {
        request.onFileCoverGenerated?.call('', true, type);
      }
