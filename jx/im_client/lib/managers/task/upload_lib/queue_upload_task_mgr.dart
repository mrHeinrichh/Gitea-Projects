...
class QueueUploadTaskMgr extends ScheduleTask {
  ...
  clear() async {
    for (QueueUploadTask task in _taskList) {
...
class QueueUploadTask implements Comparable<QueueUploadTask> {
  ...
  final Future<bool> Function(CancelToken cancelToken, Duration timeout) task;
  ...
  Future<bool> _runWithTimeout() {
    return Future.any([
      task(cancelToken, timeout),
      Future.delayed(timeout).then(
          (_) => throw TimeoutException('QueueUploadTask _runWithTimeout 189')),
