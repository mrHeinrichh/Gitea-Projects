...
class UploadChunk extends UploadChunkBase {
  ...
  Map<String, dynamic> toJson() {
    return {
      ...
      "is_encrypt": (storageType?.value == StorageType.avatar.value ||
              storageType?.value == StorageType.video.value)
          ? false
          : isEncrypt, // 是否加密
      "sourceFilePath": sourceFilePath, // 源文件地址
      "totalBytes": totalBytes, // 总字节数
      "storage_type": storageType?.value ?? StorageType.document.value,
    };
  }
...
abstract class UploadChunkBase implements JsonSerializable {
  ...
  bool isEncrypt = Config().is_upload_encrypt;
...
Future<UploadResponseData> _uploadPostSub(
  ...
  if (cancelToken.isCancelled) {
    SumUploadAnalytics.sharedInstance.updateUploadStatistic(
      AnalyticsHelper.getFileType(request?.totalBytes),
      uploadFailedKey: uploadPhase,
    );
    return UploadResponseData(false);
  }
  try {
    final int startTime = DateTime.now().millisecondsSinceEpoch;
    final response = await CustomRequest.doPost(
      url,
      data: request?.toJson(),
      printBody: true,
    );
    if (response.success()) {
      SumUploadAnalytics.sharedInstance.updateUploadStatistic(
        AnalyticsHelper.getFileType(request?.totalBytes),
        uploadSuccessKey: uploadPhase,
        apiPresignDuration: uploadPhase == UploadAnalytics.uploadPresign
            ? DateTime.now().millisecondsSinceEpoch - startTime
            : null,
        apiFinishDuration: uploadPhase == UploadAnalytics.uploadFinish
            ? DateTime.now().millisecondsSinceEpoch - startTime
            : null,
      );
...
class UploadResponseData {
  ...
  UploadResponseData(this.success, {this.data, this.err, this.duration}) {
  ...
  Future<HandleMsg> handle(T request) async {
    ...
    try {
      ...
      if (anyFail) {
        SumUploadAnalytics.sharedInstance.updateUploadStatistic(
          AnalyticsHelper.getFileType(request.totalBytes),
          uploadFailedKey: UploadAnalytics.uploading,
        );
      ...
      SumUploadAnalytics.sharedInstance.updateUploadStatistic(
        AnalyticsHelper.getFileType(request.totalBytes),
        uploadSuccessKey: UploadAnalytics.uploading,
        apiUploadingDuration: DateTime.now().millisecondsSinceEpoch - startTime,
      );
      return HandleMsg(
        true,
