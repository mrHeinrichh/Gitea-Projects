...
class VideoUploadRequest extends UploadChunk {
  ...
  VideoUploadRequest(super.sourceFilePath,
      ...
      this.onStatusChange,
      super.fileType,
      super.storageType = StorageType.video});
}
...
enum VideoResCategory {
  category144('144', '120K', 3000),
  category240('240', '250K', 3000),
  category360('360', '500K', 3000),
  category480('480', '1000K', 3000),
  category720('720', '2000K', 3000),
  category1080('1080', '6000K', 6000);
...
class VideoCompressHandler
  ...
  Future<HandleMsg> handle(VideoUploadRequest request) async {
    ...
    if (Platform.isMacOS) {
      ...
      } else if (compressedFile.path != request.sourceFilePath) {
    ...
    if (!targetCompressFileExist) {
      ...
      logMsg += '7、检查编码格式 - ';
      if (bitrate > (videoCategory.bitRateValue * 1000)) {
        ...
        if (exist == false || request.cancelToken.isCancelled) {
          return HandleMsg(
            false,
            message:
  ...
  VideoResCategory getNearestVideoCategory(int height) {
    return switch (height) {
      1080 => VideoResCategory.category1080,
      720 => VideoResCategory.category720,
      480 => VideoResCategory.category480,
      360 => VideoResCategory.category360,
      240 => VideoResCategory.category240,
      _ => VideoResCategory.category144,
    };
  }
}
...
class VideoRequestUploadUrlHandler
  ...
  Future<HandleMsg> handle(VideoUploadRequest request) async {
    ...
    request.uploadResponseData = await uploadPost(
      ...
      request: request,
      uploadPhase: UploadAnalytics.uploadPresign,
    );
...
class VideoRequestCompositionHandler
  ...
  Future<HandleMsg> handle(VideoUploadRequest request) async {
    ...
    request.uploadResponseData = await uploadPost(
      ...
      cancelToken: request.cancelToken,
      request: request,
      uploadPhase: UploadAnalytics.uploadFinish,
    );
    if (!request.uploadResponseData!.success) {
      return HandleMsg(
        false,
