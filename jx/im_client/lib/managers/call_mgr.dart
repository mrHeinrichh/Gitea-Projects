...
class CallMgr extends BaseMgr {
  ...
  final firstRemoteVideoFrameDone = false.obs;
  ...
  final iosCallInBackground = false.obs;
  ...
  Future<void> _onSocketOpen(a, b, c) async {
    ...
    // pdebug("SocketTryingToClose=====> _onSocketOpen");
  }
  ...
  void getCallServerStatus(String channelId) async {
    try {
      ...
      if (callInfo.inCall) {
        final call = callInfo.call;
        if (call != null) {
          _rtcEncryptKey = call.rtcEncryptKey;
          doCallChangePorcess(call);
        }
      } else {
  ...
  floatingWindowOnTap({bool isBack = false}) {
    pdebug(
        "floatingWindowOnTap======> iosInBG: ${iosCallInBackground.value}, isBack: $isBack, isMin: ${isMinimized.value} floatIsMe: ${floatWindowIsMe.value}");
    ...
    if (isMinimized.value) {
      if (!isBack) {
        ...
        if (!selfCameraOn.value && !friendCameraOn.value ||
          ...
          if (Platform.isAndroid) {
            // 清空之前的native的缓存view
            _methodChannel.invokeMethod("resetNativeView");
          }
        }
      } else {
  ...
  Future<void> _handleEventWaitingState(CallEvent event) async {
    switch (event) {
      ...
      case CallEvent.CallConnected:
        ...
        resetNotification();
        if (!isInviter) {
          bool result = await updateServerStatus(event);
          if (result) {
            joinChannel();
          } else {
            handleEvent(CallEvent.CallNetException);
        ...
        if (!isVoiceCall && !isMinimized.value) {
          CallFloat().showAppFloatingWindow();
          floatingWindowOnTap();
        }
        if (isVoiceCall) {
          _methodChannel.invokeMethod("playPickedSound");
        }
        _methodChannel.invokeMethod("stopDialingSound");
  ...
  Future<void> initCall(String sender) async {
    ...
    if (!isInviter &&
        !isCallKit &&
        notBlank(rtcChannelId) &&
        currentState.value == CallState.Init) {
      objectMgr.logMgr.logCallMgr.addMetrics(
        LogCallMsg(
          deviceId: _logId,
          msg: CallLogInfo(
      ...
      // 延迟一会是等待所有通话需要的参数进行初始化完成
      if (Platform.isIOS || Platform.isAndroid) {
        Future.delayed(
            const Duration(milliseconds: 300), () => openAnswerWidget());
      }
    }

    objectMgr.socketMgr.setAllowClose(false);

    final socketIsDisconnected = objectMgr.socketMgr.socket == null ||
        objectMgr.socketMgr.socket?.open == false;
    // pdebug(
    //     "SocketTryingToClose=====> initCall: $socketIsDisconnected, $isCallKit");
    // 通话过程中阻止socket后台90秒后断掉的的逻辑，不然切后台去电话无法监听到结束的长连接推送
    if (socketIsDisconnected && isCallKit) {
      objectMgr.socketMgr.init(objectMgr.loginMgr.account!.token);
    }
  }

  openAnswerWidget() async {
    if (isCallCompleted(rtcChannelId) || isCallKit) return;
  ...
  _playIOSRing() async {
    final isIOSForeground = await _methodChannel.invokeMethod("isInForeground");
    if (!isCallKit && isIOSForeground) {
      _methodChannel.invokeMethod("playRingSound");
    }
  }

  playAndroidRing() async {
    final isInForeApp = await objectMgr.pushMgr.getAppState();
    //切后台的时候收到来电通知，需要等200毫秒，因为唤起常驻消息的时机慢于长连接的时机
    if (!isInForeApp) {
      Future.delayed(const Duration(milliseconds: 200), () {
        _playAndroidRing(isInForeApp);
      });
    } else {
      _playAndroidRing(isInForeApp);
    }
  ...
  _playAndroidRing(bool inAppFore) async {
    objectMgr.logMgr.logCallMgr.addMetrics(
      LogCallMsg(
        deviceId: _logId,
        msg: CallLogInfo(
          channelId: rtcChannelId,
          method: "playAndroidRing",
          state: currentState.value,
          opt: 'inAppFore:${inAppFore.toString()}',
        ).toString(),
        mediaType: getLogCallInfoStr(),
      ),
    );

    if (objectMgr.callMgr.currentState.value == CallState.Connecting ||
        objectMgr.callMgr.currentState.value == CallState.InCall) {
      return;
    }

    final isNotificationRunning =
        await _methodChannel.invokeMethod("NotificationServiceRunning");
    if (!isNotificationRunning) {
      pdebug(
          "isNotificationRunning===> isAppFore: $inAppFore, isNotificationRunning: $isNotificationRunning");
      _methodChannel.invokeMethod("playRingSound");
    }
  }
  ...
  Future<void> initAgoraEngine() async {
    try {
      ...
      String avatarUrl = await getAvatarUrl(opponent);
      _methodChannel.invokeMethod(
        'setupAgoraEngine',
        {
  ...
  Future<String> getAvatarUrl(User? user) async {
    String avatarUrl = "";
    if (user != null && notBlank(user.profilePicture)) {
      String? avatarLocalUrl = downloadMgrV2.getLocalPath(user.profilePicture,
          mini: Config().headMin);
      if (avatarLocalUrl != null) {
        avatarUrl = avatarLocalUrl;
      } else {
        avatarUrl = "${serversUriMgr.download2Uri}/${user.profilePicture}";
        avatarUrl = getMiniAvatarUrl(avatarUrl);
        avatarUrl = (await LocalHttpServer.getLocalproxy(Uri.parse(avatarUrl)))
            .toString();
      }
    }
    return avatarUrl;
  ...
  String getMiniAvatarUrl(String avatarUrl) {
    if (!avatarUrl.contains(".")) {
      return avatarUrl;
    }

    int dotIndex = avatarUrl.lastIndexOf(".");
    String namePart = avatarUrl.substring(0, dotIndex);
    String extensionPart = avatarUrl.substring(dotIndex);

    return '${namePart}_${Config().headMin}$extensionPart';
  }

  onUserJoined(int uid) {
    if (!objectMgr.userMgr.isMe(uid)) {
      agoraViewController?.doFriendJoined();
  ...
  Future<int> getAndroidTargetVersionApi() async {
    return await _methodChannel.invokeMethod('getAndroidTargetVersion', {});
  }
  ...
  Future<void> nativeCallback(MethodCall call) async {
    ...
    switch (call.method) {
      ...
      case 'callKitIncomingCall':
        ...
        int friendId = data["sender_id"];
        rtcChannelId = data['rtc_channel_id'];
        isVoiceCall = data['isVideoCall'] == 0;
        selfCameraOn.value = !isVoiceCall;

        pdebug(
            "callKitIncomingCall===> chatID:$chatID, friendId: $friendId, rtcChannelId:$rtcChannelId");

        List results = await Future.wait([
          objectMgr.chatMgr.getChatByFriendId(friendId),
          objectMgr.userMgr.loadUserById(friendId)
        ]);
      ...
      case "onFirstRemoteVideoFrame":
        firstLocalVideoFrameDone.value = true;
        firstRemoteVideoFrameDone.value = true;
        break;
      case "cameraIsInit":
      ...
      case "audioMuted":
        ...
        break;
      case "iosCallInBackground":
        Map data = call.arguments;
        iosCallInBackground.value = data["isIosCallInBackground"] ?? false;
        break;
      default:
        break;
    }
  ...
  void updateCallFloat() {
    // 对方没有视频流和我自己也没开视频就要关闭小浮窗
    if (!isMinimized.value &&
        (iosCallInBackground.value || !friendCameraOn.value) &&
        !selfCameraOn.value) {
      CallFloat().closeAllFloat();
    }
  }
  ...
  void startInCallTimer() {
    ...
    _inCallTimer = Timer.periodic(const Duration(seconds: 1), (timer) {
      ...
      if (_appLifecycleState != AppLifecycleState.resumed) {
        objectMgr.scheduleMgr.heartBeatTask.resetDelayCount();
      }
    });
  }
  ...
  void closeCall(CallEvent event) async {
    ...
    // 恢复socket后台90秒后断掉的
    objectMgr.socketMgr.setAllowClose(true);

    shouldShowNativeVideo.value = false;
    iosCallInBackground.value = false;
    ...
    firstRemoteVideoFrameDone.value = false;
    ...
    if (agoraViewController?.animationController?.status ==
            AnimationStatus.completed &&
        agoraViewController?.animationController?.isAnimating == false) {
      agoraViewController?.showButton(customize: true, animate: false);
    }
    ...
    if (isInviter) {
      ...
      if (currentState.value != CallState.InCall) {
        if (event == CallEvent.CallOptReject ||
          ...
          bool isForeground = await getForeground();
          if (isForeground) {
  ...
  Future<bool> getForeground() async {
    bool isForeground = true;
    if (Platform.isIOS) {
      isForeground = await _methodChannel.invokeMethod("isInForeground");
    } else if (Platform.isAndroid) {
      isForeground = await objectMgr.pushMgr.getAppState();
    }
    return Future.value(isForeground);
  }
  ...
  * */
  ...
  Future<bool> updateServerStatus(CallEvent event, {int attempts = 0}) async {
  ...
  Future<bool> requestPermission() async {
    ...
    try {
      ...
      pdebug("requestPermission Failed====> $e");
    }
    ...
    if (!(statusCam.isGranted && statusMic.isGranted)) {
      ...
      showCustomBottomAlertDialog(
        ...
        confirmText: localized(goToSettings),
        cancelText: localized(discardButton),
  ...
  requestFloatingPermission() async {
    if (Platform.isAndroid) {
      ...
      if (!result && !optPermissionExist) {
        ...
        showCustomBottomAlertDialog(Get.context!,
            isDismissible: false,
            title: localized(callAccessNeeded),
            ...
            cancelText: localized(discardButton),
            confirmTextColor: themeColor,
            cancelTextColor: themeColor, onConfirmListener: () async {
  ...
  Future<bool> setAudioSound(SoundType type) async {
    objectMgr.logMgr.logCallMgr.addMetrics(
      LogCallMsg(
        ...
        msg: CallLogInfo(
          channelId: rtcChannelId,
          method: "setAudioSound",
          state: currentState.value,
          opt: "soundType: ${type.code}",
        ).toString(),
        mediaType: getLogCallInfoStr(),
      ),
    );
