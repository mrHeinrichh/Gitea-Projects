...
class PushManager with WidgetsBindingObserver {
      ...
      case 1:
        ...
        await Future.delayed(Duration(milliseconds: waitTime), () {
          if (!Get.currentRoute.contains("${chat.chat_id}")) {
            try {
              objectMgr.pushMgr
                  .ifMinimizedCallView("notificationRouting1", chat);
              Get.until(
                (route) =>
  ...
  Future showNotification(
    ...
    if (isNotMute) {
      if (Platform.isMacOS) {
        ...
        // MacOs系统的，只需要调用系统的通知，app内的在线通知不用管
        return;
      }
      ...
      if (Platform.isAndroid) {
        ...
        if (pathList != null &&
            pathList.isNotEmpty &&
            pathList.last.split('.').first != null) {
          path = 'noti_' + pathList.last.split('.').first;
        } else {
          path =
              'noti_f970414c3e2d6583afefecd166e3471b'; // default notification sound
        }
        ...
        flutterLocalNotificationsPlugin.show(
          ...
          NotificationDetails(
            android: AndroidNotificationDetails(
              ...
              sound:
                  isInCall ? null : RawResourceAndroidNotificationSound(path),
            ),
          ),
          payload: payLoad,
        );
  ...
  _localEnablePushServices() async {
    if (!objectMgr.loginMgr.isDesktop) {
      pdebug("enable push start");
      _methodChannel.invokeMethod('enablePush');
    }
  }

  _localDisablePushServices() async {
    if (!objectMgr.loginMgr.isDesktop) {
      pdebug("disable push start");
      _methodChannel.invokeMethod('disablePush');
    }
  }
  ...
  void clearNotification() => _methodChannel.invokeMethod("clearNotification");
