...
class FavouriteMgr extends BaseMgr {
  ...
  void addMessageToFavourite(List<Message> msgList, Chat chat) async {
    ...
    imBottomToast(Get.context!,
        title: localized(favouriteAdded), icon: ImBottomNotifType.success);
    ...
    switch (msg.typ) {
      ...
      case messageTypeLink:
        MessageText messageText =
            msg.decodeContent(cl: msg.getMessageModel(msg.typ));
  ...
  Future<FavouriteFile?> _uploadFile(
    ...
    try {
      final String? fileUrl = await documentMgr.upload(
        file.path,
        storageType: StorageType.favorite,
