...
class StickerMgr extends BaseMgr {
  ...
  static const String keyChatPopMenuCloseEvent = 'keyChatPopMenuCloseEvent';
  ...
  void _initFetchedState() {
    final isFetchedState = objectMgr.localStorageMgr.read(
          LocalStorageMgr.STICKERS_AND_GIFS_FETCHED,
        ) ??
        false;

    isFetched.value = isFetchedState;
  }

  @override
  Future<void> initialize() async {
    getAllStickersFromLocal();
    getAllGifsFromLocal();
    getRecentEmojis();
    getRecentStickers();
  ...
  ///获取所有原本的贴纸
  Future<void> getAllGifsFromLocal() async {
    List gifCollections =
        objectMgr.localStorageMgr.read(LocalStorageMgr.ALL_GIFS) ?? [];

    gifCollectionList =
        gifCollections.map((collection) => Gifs.fromJson(collection)).toList();

    event(this, eventGifChange);
  }

  void _setFetchedState() {
    objectMgr.localStorageMgr
        .write(LocalStorageMgr.STICKERS_AND_GIFS_FETCHED, true);
    isFetched.value = true;
  }

  Future<void> _getAllStickersAndGifs() async {
  ...
  void fetch({bool isForce = false}) {
    if (isForce == true) {
      _getAllStickersAndGifs();
      return;
    }

    final lastTimeMillis = objectMgr.localStorageMgr.read(
          LocalStorageMgr.LAST_STICKERS_AND_GIFS_FETCH,
        ) ??
        0;

    final now = DateTime.now();
    final lastDateTime = DateTime.fromMillisecondsSinceEpoch(lastTimeMillis);

    if (now.difference(lastDateTime).inDays > 0) {
      _getAllStickersAndGifs();
  ...
  Future<void> downloadToLocal() async {
    ...
    for (StickerCollection stickerCollection in stickerCollectionList) {
      ...
      downloadMgrV2.download(
        path,
        downloadType: DownloadType.smallFile,
      );
      // downloadMgr.downloadFile(path);
  ...
  onChatPopMenuCloseEvent(bool popupEnabled) {
    event(this, keyChatPopMenuCloseEvent, data: popupEnabled);
  }
