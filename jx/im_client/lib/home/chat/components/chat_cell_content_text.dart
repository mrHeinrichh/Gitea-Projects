...
class ChatCellContentTextState extends State<ChatCellContentText> {
  ...
  final RxString draftString = ''.obs;
  ...
  void initState() {
    ...
    if (objectMgr.loginMgr.isDesktop) {
      objectMgr.chatMgr.on(
        '${widget.chat.id}_${ChatMgr.eventDraftUpdate}_instant',
        _onDraftUpdateInstant,
      );
    }
  ...
  @override
  void didUpdateWidget(ChatCellContentText oldWidget) {
  ...
  String callIconName(Message? lastMessage) {
    // 群组消息不用展示打电话的图标，这里主要是拼接电话图标的图片名字
    String contentString = lastMessage?.content ?? '';
    int? isVideoCall;
    int? inviter;
    if (contentString.isNotEmpty) {
      try {
        ...
        // 检查并赋值 is_videocall
        if (content['is_videocall'] is int) {
          isVideoCall = content['is_videocall'] as int;
        }
        // 检查并赋值 inviter
        if (content['inviter'] is int) {
          inviter = content['inviter'] as int;
        }
      } catch (e) {
        debugPrint('解析 JSON 出错: $e');
      }
    }
    if (lastMessage != null &&
        !widget.chat.isGroup &&
        isVideoCall != null &&
        inviter != null) {
      String imageName = isVideoCall == 1 ? "icon_video_call" : "icon_call";
      String callerReceiver =
          objectMgr.userMgr.isMe(inviter) ? "_caller" : "_receiver";
      ...
      switch (lastMessage.typ) {
        ...
        default:
          takenCallOrRejected =
              objectMgr.userMgr.isMe(inviter) ? "" : "_rejected";
          break;
      }
      return "$imageName$takenCallOrRejected$callerReceiver";
    }
    return '';
  }
  ...
  void updateContent() {
    ...
    if (draftModel != null) draftString.value = draftModel.input;
  ...
  void _onDraftUpdateInstant(_, __, Object? data) {
    if (data == null || data is! Map<String, dynamic>) return;

    if (data['chat_id'] == null || data['chat_id'] != widget.chat.id) return;

    draftString.value = data['text'];
  }

  String prepareContentString(Message message, int curUID) {
    bool isMe = objectMgr.userMgr.isMe(curUID);
    if (message.isEncrypted) {
      return getEncryptionText(message, widget.chat);
    }
    ...
    if (msg != null) {
      ...
      switch (type) {
        ...
        case messageTypeFace:
          String url = json.decode(lastMessage!.content)['url'];
          return url;
        default:
          return null;
      }
  ...
  Widget _buildMsgContent() {
    ...
    return Obx(() {
      ...
      return RichText(
        ...
        text: TextSpan(
          children: <InlineSpan>[
            if ((lastMessage?.isCalling ?? false) &&
                callIconName(lastMessage).isNotEmpty)
              WidgetSpan(
                alignment: PlaceholderAlignment.middle,
                child: Padding(
                  padding: const EdgeInsets.only(right: 4.0),
                  child: SvgPicture.asset(
                    'assets/svgs/${callIconName(lastMessage)}.svg',
                    height: 20.0,
                    width: 20.0,
                  ),
                ),
