...
class User extends RowObject with EventDispatcher {
  ...
  factory User.fromJson(Map<String, dynamic> json) {
    ...
    user.profilePictureGaussian = json["profile_pic_gaussian"] ?? '';
    ...
    user.groupTags = json["group_tags"] != null
        ? json["group_tags"] is String ? List<String>.from(jsonDecode(json["group_tags"])) : List<String>.from(json["group_tags"])
        : <String>[];
    if (json["request_at"] != null) user.requestTime = json["request_at"];
    user.localName = json['local_name'] ?? ''; // Local contact name
    user.localPhoneNumbers = json['local_phone_numbers'] ?? '';
    user.friendTags = json["friend_tags"] != null
  ...
  factory User.fromGroupMember(Map<String, dynamic> json) {
    ...
    user.profilePictureGaussian = json["icon_gaussian"] ?? '';
    user.lastOnline = json["last_online"] ?? 0;
  ...
  Map<String, dynamic> groupMemberToJson() {
    return {
      ...
      'user_name': nickname,
      'icon': profilePicture,
      'icon_gaussian': profilePictureGaussian,
  ...
  String get profilePictureGaussian => getValue('profile_pic_gaussian', '');

  set profilePictureGaussian(String v) => setValue('profile_pic_gaussian', v);
  ...
  // 用户组标签
  List<String>? get groupTags => getValue<List<String>>('group_tags', <String>[]);
  ...
  // 标签
  List<int>? get friendTags => getValue<List<int>>('friend_tags', <int>[]);

  set friendTags(List<int>? value) {
    setValue('friend_tags', value);
  }
  ...
  String get localName => getValue('local_name', '');

  set localName(String v) => setValue('local_name', v);

  String get localPhoneNumbers => getValue('local_phone_numbers', '');

  set localPhoneNumbers(String v) => setValue('local_phone_numbers', v);
