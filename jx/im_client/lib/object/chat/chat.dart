...
class Chat extends RowObject with EventDispatcher {
  ...
  int get cover_idx => getValue('cover_idx', 0);

  set cover_idx(int v) => setValue('cover_idx', v);

  String get chat_key => getValue('chat_key', '');

  set chat_key(String v) => setValue('chat_key', v);

  String get active_chat_key => getValue('active_chat_key', '');

  set active_chat_key(String v) => setValue('active_chat_key', v);

  int get chatKeyRound =>
      isChatKeyValid ? int.parse(chat_key.split("_").last) : 0;

  int get activeKeyRound =>
      isActiveChatKeyValid ? int.parse(active_chat_key.split("_").last) : 0;

  String get chatKey => isChatKeyValid ? chat_key.split("_").first : "";

  String get activeChatKey =>
      isActiveChatKeyValid ? active_chat_key.split("_").first : "";

  bool get isChatKeyValid =>
      notBlank(chat_key) &&
      chat_key != EncryptionMgr.decryptFailureEmblem &&
      chat_key.contains("_");

  bool get isActiveChatKeyValid =>
      notBlank(active_chat_key) &&
      active_chat_key != EncryptionMgr.decryptFailureEmblem &&
      active_chat_key.contains("_");

  //当前chat 加密round
  int get round => getValue('round', 0);

  set round(int v) => setValue('round', v);
  ...
  bool get isChatTypeMiniApp => typ == chatTypeMiniApp;

  /// 是否是加密聊天室
  bool get isEncrypted =>
      (flag & ChatEncryptionFlag.encrypted.value) ==
      ChatEncryptionFlag.encrypted.value;

  // 是否曾经开启加密
  bool get hasPreviousEncryption =>
      isEncrypted || flag == ChatEncryptionFlag.previouslyEncrypted.value;
  ...
  /// 是否可见
  bool get isVisible =>
      delete_time <= last_time && (typ != 0) && (cover_idx != msg_idx);
  ...
  void updateChatKey(String chatKey, int round) {
    if (chatKey.isEmpty || chatKey == EncryptionMgr.decryptFailureEmblem) {
      chat_key = "";
      return;
    }
    String key = [chatKey, round.toString()].join("_");
    chat_key = key;
  }

  void updateActiveChatKey(String chatKey, int round) {
    if (chatKey.isEmpty || chatKey == EncryptionMgr.decryptFailureEmblem) {
      active_chat_key = "";
      return;
    }
