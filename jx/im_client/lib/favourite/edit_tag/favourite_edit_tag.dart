...
class FavouriteEditTag extends StatefulWidget {
  final List<String> tagDataList;

  const FavouriteEditTag({
    super.key,
    required this.tagDataList,
  });

  @override
  State<FavouriteEditTag> createState() => _FavouriteEditTagState();
}

class _FavouriteEditTagState extends State<FavouriteEditTag> {
  ...
  RxList<MomentInputTagModel> tagList =
      <MomentInputTagModel>[].obs; // add to tag favourite
  RxList<MomentInputTagModel> mainTagList =
      <MomentInputTagModel>[].obs; // submit to main tag
  ...
  void _onInputChanged() {
    ...
    if (isDelete) {
      ...
      if (inputController.text.trim().isEmpty &&
          tagList.isNotEmpty &&
          !tagList.last.selected) {
        tagList[tagList.length - 1] = tagList[tagList.length - 1].copyWith(
          selected: true,
        );
