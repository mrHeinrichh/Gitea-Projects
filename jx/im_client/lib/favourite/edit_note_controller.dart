...
class EditNoteController extends GetxController {
  ...
  showBottomPopup(BuildContext context, MediaOption mediaOption) async {
    ...
    try {
      if (mediaOption == MediaOption.gallery ||
        ...
        permissionStatus = await onPrepareMediaPicker(
    ...
    if (mediaOption == MediaOption.document && permissionStatus) {
      Future.delayed(const Duration(milliseconds: 200), () {
        final filePickerController = Get.findOrPut(FilePickerController());
    ...
    showModalBottomSheet(
      ...
      builder: (BuildContext ctx) {
        if (!permissionStatus) {
          var str = mediaOption == MediaOption.gallery ? "photo" : "file";
          return NoPermissionView(
            title: localized(files),
            imageUrl: 'assets/svgs/noPermission_state_$str.svg',
  ...
  onPhoto(BuildContext context) async {
    ...
    var b =
        await Permissions.request([Permission.camera, Permission.microphone]);
    if (!b) return;

    var c = await onPrepareMediaPicker();
    if (!c) return;
  ...
  onAssetPickerChanged() {
    if (assetPickerProvider == null) return;
