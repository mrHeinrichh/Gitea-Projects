...
class FavouriteData {
  ...
  factory FavouriteData.fromJson(Map<String, dynamic> json) {
    ...
    List<FavouriteDetailData> content = [];
    if (notBlank(json['data'])) {
      var decode = jsonDecode(json['data']);
      if (decode is List) {
        content = decode
            .map((e) => FavouriteDetailData.fromJson(e as Map<String, dynamic>))
            .toList();
      } else if (decode is Map) {
        content = [
          FavouriteDetailData.fromJson(decode as Map<String, dynamic>)
        ];
      }
    }
