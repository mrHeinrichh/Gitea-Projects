...
class ReelSearchView extends GetView<ReelSearchController> {
  ...
  final RxBool dragTrigger = false.obs;
  ...
  Widget build(BuildContext context) {
    return GetBuilder(
      ...
      builder: (c) {
        Widget w = Column(
          children: [
            ReelSearchInput(
              ...
              hasAutoFocus: false,
              onBackClick: _onBackTriggered,
              onClearClick: () => controller.clearInput(),
              onSearchClick: () => controller.onSearch(controller.searchController.text),
            ),
            Expanded(
              child: Obx(
        ...
        if (Platform.isAndroid || !controller.isSearching.value) {
          //add popscope
          w = WillPopScope(
              onWillPop: () async {
                return _onBackTriggered();
              },
              child: w);
        }
        ...
        return GestureDetector(
          ...
          onHorizontalDragUpdate: (details) {
            if (!controller.isSearching.value && !Platform.isAndroid && details.delta.dx > 5) {
              //add popscope
              if (!dragTrigger.value) {
                dragTrigger.value = true;
                _onBackTriggered();
              }
            }
          },
          onHorizontalDragEnd: (_) {
            dragTrigger.value = false;
          },
          child: Scaffold(
                        ...
                        child: VisibilityDetector(
                          ...
                          onVisibilityChanged: (visibilityInfo) {
                            if (visibilityInfo.visibleFraction == 1.0) {
                              controller.resultTagRect = visibilityInfo.visibleBounds;
                            }
                          },
                          child: Padding(
        ...
        child: Container(
          ...
          child: Text(
            ...
            style: jxTextStyle.textStyleBold14(
              color:
                  (controller.searchKeyword.value == tag) ? colorTextPrimary : colorTextSecondary,
