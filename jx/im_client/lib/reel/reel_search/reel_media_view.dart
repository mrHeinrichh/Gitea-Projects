...
class ReelMediaView extends StatefulWidget {
  ...
  final Function(TencentVideoStream)? onReturn;
...
class _ReelMediaViewState extends State<ReelMediaView> with WidgetsBindingObserver {
  ...
  void initState() {
    ...
    if (widget.searchController != null) {
      searchTextEditingController.text = widget.searchController!.searchController.text;
    }

    videoStreamMgr = objectMgr.tencentVideoMgr.getStream();
    videoStreamMgr.currentIndex.value = widget.startingIndex;
    videoStreamSubscription = videoStreamMgr.onStreamBroadcast.listen(_onVideoUpdates);

    if (widget.startingStream != null) {
      widget.startingStream?.stopRemoval = true;
      videoStreamMgr.updateStream(widget.startingStream!);
      if (widget.startingStream?.controller.previousState == TencentVideoState.PLAYING) {
      ...
      currentVideoStream.value = widget.startingStream;
    }
  ...
  void didChangeMetrics() {
    ...
    isLandscape.value =
        WidgetsBinding.instance.platformDispatcher.views.first.physicalSize.aspectRatio > 1;
  }
  ...
  _dismissPage() {
    if (widget.onReturn != null) {
      ...
      widget.onReturn?.call(currentVideoStream.value!);
    }
                  ...
                  direction: isSearchMode.value
                      ...
                      : currentPage.value == 0
                          ...
                          : currentPage.value == computedAssets.length - 1
                              ...
                              : DismissiblePageDismissDirection.startToEnd, // 其余页面，右滑关闭
                  child: Stack(
                    children: [
                      Positioned(
                        top: 0,
                        ...
                        child: PreloadPageView.builder(
                          ...
                          itemBuilder: (context, index) {
                            ...
                            return ReelPreview(
                              ...
                              onReturn: () {
                                widget.onReturn != null ? _dismissPage() : Get.back();
                              },
                              onReturnFromSave: widget.onReturnFromSave,
                              commentHasFocus: isCommentTextFieldFocus.value || isSearchMode.value,
                              transferParams: () {
                      ...
                      Stack(
                        children: [
                          Positioned.fill(
                              top: isSearchMode.value ? 0 : MediaQuery.of(context).viewPadding.top,
                              child: Column(
                                children: <Widget>[
                                  if (widget.searchController == null)
                                    Row(
                                      ...
                                      children: [
                                        Expanded(
                                          child: ReelSearchInput(
                                            ...
                                            onChanged: (value) {
                                              widget.searchController!.getSearchCompletion(value);
                                            },
                                            onClick: () {
                                              _enterSearchMode();
                                            },
                                  ...
                                  if (isSearchMode.value)
                                    Expanded(
                                      child: widget.searchController!.getSearchContent(
  ...
  _onSearch(String text) {
    if (widget.searchController!.searchController.text != text) {
      _dismissPage();
  ...
  _enterSearchMode() {
    ...
    if (currentVideoStream.value != null) {
      _isControllerPlaying = currentVideoStream.value?.state.value == TencentVideoState.PLAYING;
  ...
  void _preloadVideos(int index) async {
    for (int i = index - videoCacheRange; i <= index + videoCacheRange; i++) {
      ...
      if (data != null) {
        if (objectMgr.tencentVideoMgr.currentStreamMgr?.getVideoStream(i) == null) {
          ...
          objectMgr.tencentVideoMgr.currentStreamMgr?.addController(config, index: i);
        }
      }
    }
  }
