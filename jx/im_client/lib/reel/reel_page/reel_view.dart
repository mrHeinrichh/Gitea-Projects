      ...
      child: GestureDetector(
        ...
        onHorizontalDragUpdate: (details) {
          if (details.delta.dx < -5) {
            if (controller.currentTab.value == ReelController.RECOMMEND_TAB) {
              controller.onNavigation();
              var currentPost = controller.postList[controller.currentPage];
              if (objectMgr.userMgr
                  .isMe(currentPost.creator.value!.id.value!)) {
                controller.onBottomTap(2);
              } else {
                Get.toNamed(
                  RouteName.reelProfileView,
                  preventDuplicates: false,
                  arguments: {
                    "userId": currentPost.creator.value!.id.value!,
                    "onBack": (toDismiss) {
                    ...
                    child: ColoredBox(
                      ...
                      child: Obx(
                        () => PreloadPageView.builder(
                          ...
                          onScrollNotification: controller.onScrollNotification,
                          preloadPagesCount: 2,
