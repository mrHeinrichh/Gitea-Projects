...
class MomentMyPostViewerPage extends StatefulWidget {
  ...
  final MomentMyPostsController momentMyPostsController;
  const MomentMyPostViewerPage({
    super.key,
    required this.index,
    required this.momentMyPostsController,
  });
...
class _MomentMyPostViewerPageState extends State<MomentMyPostViewerPage>
  ...
  void initState() {
    ...
    postList.assignAll(widget.momentMyPostsController.postList);
    ...
    for (int i = 0; i < widget.index; i++) {
      if (widget.momentMyPostsController.postList[i].post!.content!.assets == null ||
          widget.momentMyPostsController.postList[i].post!.content!.assets!.isEmpty) {
        removedCount++;
      }
    }
  ...
  (String, String, String?, int, int) _getPreloadParams(int index) {
    if (index < 0 || index > _allAssets.length-1){
      return ("", "", null, 0, 0);
    }
  ...
  Message _onForwardMedia(MomentContentDetail assetsMedia, int media) {
    ...
    if (media == 0) {
      ...
      messageVideo.second = duration ~/ 1000;
  ...
  void updateMyPosts(int aAction, var post) {
    if (aAction == POST_ACTION_UPDATE) {
      for (int i = 0; i < widget.momentMyPostsController.postList.length; i++) {
        if (widget.momentMyPostsController.postList[i].post!.id == post.post?.id) {
          widget.momentMyPostsController.postList[i] = post;
          break;
        }
      }
      ...
      for (int i = 0; i < widget.momentMyPostsController.postList.length; i++) {
        if (widget.momentMyPostsController.postList[i].post!.id == post.post?.id) {
          widget.momentMyPostsController.postList.removeAt(i);
          break;
        }
      }
    }
