...
class CreateTagsBottomSheetController extends GetxController {
  CreateTagsBottomSheetController();
  ...
  RxMap<int, List<User>> allTagByGroup = <int, List<User>>{}.obs;

  RxList<Tags> tagsList = <Tags>[].obs;

  RxList<Tags> selectedMembers = <Tags>[].obs;
  RxList<AZItem> azFilterTagsList = <AZItem>[].obs;
  ...
  void updateAZFriendList() {
    azFilterTagsList.value = tagsList
        .map(
          (e) => AZItem(
            user: objectMgr.userMgr.mainUser,
            tag: convertToPinyin(objectMgr.userMgr
                    .getUserTitle(objectMgr.userMgr.mainUser)[0])[0]
                .toUpperCase(),
          ),
        )
        .toList();

    SuspensionUtil.setShowSuspensionStatus(azFilterTagsList);
  }
  ...
  Future<void> getFriendList() async {
    tags = objectMgr.tagsMgr.allTags;
    tagsList = tags.obs;
    updateAZFriendList();
    allTagByGroup.value = await objectMgr.tagsMgr.getAllTagByGroup();
  }

  void onSelect(BuildContext context, bool? selected, Tags tags) {
    final indexList =
        selectedMembers.indexWhere((element) => element.uid == tags.uid);

    if (indexList > -1) {
      selectedMembers.removeWhere(
        (Tags selectedTags) => selectedTags.uid == tags.uid,
      );
  ...
  getGalleryPhoto(BuildContext context) async {
    if (objectMgr.loginMgr.isDesktop) {
      ...
      PermissionState ps = await requestAssetPickerPermission();
  ...
  void switchPage(int page) {
    ...
    if (page == 1) {
      ...
      if (selectedMembers.length <= 4) {
        selectedMembers.asMap().forEach((index, tags) {
          String memberName = tags.tagName;
