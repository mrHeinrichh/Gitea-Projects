...
class CustomRequest {
  ...
  /// attempts 尝试次数
  /// maxTry 最大尝试次数
  static Future<HttpResponseBean> send(String apiPath,
    ...
    try {
      ...
      if (method == CustomRequest.methodTypePost) {
        request = await httpClient.postUrl(uri).timeout(duration,
            onTimeout: () => throw TimeoutException('Request send timeout'));
      } else if (method == CustomRequest.methodTypePut) {
        request = await httpClient.putUrl(uri).timeout(duration,
            onTimeout: () => throw TimeoutException('Request send timeout'));
      } else if (method == CustomRequest.methodTypeGet) {
        request = await httpClient.getUrl(uri).timeout(duration,
            onTimeout: () => throw TimeoutException('Request send timeout'));
      }
      ...
      var rep = await request.close().timeout(duration,
          onTimeout: () =>
  ...
  static Future<ResponseData> doPost(String apiPath,
      {dynamic data,
      int attempts = 1,
      ...
      Duration duration = const Duration(seconds: 10)}) async {
    if (Config().host == '' && requestData == null) {
      throw NetworkException(localized(waitingForNetworkConnection));
    }
    ...
    SumApiAnalytics.sharedInstance.updateAnalyticsInfo(
      apiPath,
      count: true,
    );
    ...
    try {
      ...
      HttpClientResponse rep = await request.close().timeout(
            duration,
            onTimeout: () =>
      ...
      ///response处理
      return await statusCodeProcess(apiPath, rep, body, requestData);
    } catch (e) {
      SumApiAnalytics.sharedInstance.updateAnalyticsInfo(
        apiPath,
        failed: true,
      ...
      SumApiAnalytics.sharedInstance.updateAnalyticsInfo(
        apiPath,
        latencyDuration: endTime - startTime,
      );
      httpClient.close();
    }
  }

  static Future<ResponseData> doGet(String apiPath,
      {dynamic data,
      int attempts = 1,
      int refreshAttempts = 1,
      int maxTry = maxAttempts,
    ...
    try {
      ...
      ///设置 Header
      appRequestId =
          setHeader(request, data, needToken: needToken, token: token);

      var rep = await request.close().timeout(duration,
          onTimeout: () => throw TimeoutException('Request get close timeout'));
  ...
  static String setHeader(
    ...
    Map<String, dynamic> headers = {};
    if (needToken) {
      headers['token'] = objectMgr.loginMgr.account?.token ?? token;
    }
