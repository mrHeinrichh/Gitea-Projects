...
class FormatTime {
  static String formatTimeFun(int? createTime, {bool useOnline = true}) {
    ...
    if (differenceSecond < 70) {
      ...
      if (differenceMinute == 1 && localized(myChatMinutes).contains('s')) {
        time =
            '$differenceMinute${localized(myChatMinutes).replaceFirst('s', '')}';
      } else {
        time = '$differenceMinute${localized(myChatMinutes)}';
      }
    } else if (differenceSecond >= 3600 && differenceSecond < 24 * 3600) {
      if (differenceHours == 1 && localized(myChatHours).contains('s')) {
        time =
            '$differenceHours${localized(myChatHours).replaceFirst('s', '')}';
      } else {
    ...
    if (dateStyle == DateStyle.YYYYMMDD) {
      ...
      if (year != currentYear) {
        ...
        if (locale.languageCode.toLowerCase() == 'zh') {
          return localized(yyyymmddFormat, params: [
            year.toString(),
            getMonth(createTime),
            getDay(createTime),
          ]);
        ...
        return localized(mmddFormat, params: [
          getMonth(createTime),
          getDay(createTime),
        ]);
      }
  ...
  //2021年1月1日 15:15
  static String getFullDayTime(int startime) {
    String time = '';
    DateTime cdate = DateTime.fromMillisecondsSinceEpoch(startime * 1000);
    time = '${cdate.year.toString()}${localized(myChatYear1)}'
        '${cdate.month.toString()}${localized(myChatMonth1)}'
        '${cdate.day.toString()}${localized(myChatDay2)} '
        '${cdate.hour.toString().padLeft(2, '0')}:${cdate.minute.toString().padLeft(2, '0')}';
    return time;
  }
  ...
  static String getCountTime(int time) {
    ...
    if (difference.inSeconds < 60) {
      ...
      timeAgo = '${difference.inDays} ${localized(myChatDaysAgo)}';
    }
    return timeAgo;
  }
