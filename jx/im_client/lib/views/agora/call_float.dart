...
class CallFloat {
  ...
  void onMinimizeWindow() {
    ...
    if (objectMgr.callMgr.friendCameraOn.value &&
      ...
      objectMgr.callMgr.floatWindowIsMe.value = false;
      objectMgr.callMgr.updateNativeFloatVal();
    }
  }
  ...
  getFloatingContentView() {
    return Obx(() {
      final hasFriendStream = objectMgr.callMgr.friendCameraOn.value &&
          !objectMgr.callMgr.iosCallInBackground.value &&
      ...
      if (objectMgr.callMgr.isMinimized.value) {
        ...
        if (!hasFriendStream && !objectMgr.callMgr.selfCameraOn.value) {
          return getAudioView();
  ...
  Widget getAudioView() {
    ...
    return Stack(
      children: [
        if (Platform.isAndroid) getVideoView(),
        GestureDetector(
          onTap: () => objectMgr.callMgr.floatingWindowOnTap(),
          child: Container(
            height: 200,
