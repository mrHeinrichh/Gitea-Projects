...
class QRCodeScannerController extends GetxController {
  ...
  void getDataFromQR(String? result, BuildContext context) async {
    ...
    try {
      ...
      if (userId != null && secretUrl != null) {
        ...
        String? loginData;
        if (notBlank(e2eKey) && Config().e2eEncryptionEnabled) {
          loginData = objectMgr.encryptionMgr.processE2EDesktop(e2eKey!);
        }
  ...
  void verifyPrivateKeyFromQRFlow(String result, BuildContext context) async {
    ...
    var verified = objectMgr.encryptionMgr.verifySignedKey(chat!, result);
  ...
  Future<void> saveEncryptionPrivateKey(int uid, String privateKey) async {
    ...
    if (status) {
      String publicKey = objectMgr.localStorageMgr
              .read(LocalStorageMgr.ENCRYPTION_PUBLIC_KEY) ??
          '';
      if (publicKey == '') {
      ...
      bool verified =
          objectMgr.encryptionMgr.verifyKeys(publicKey, privateKey, uid);
      ...
      objectMgr.encryptionMgr.scanQRFlow();
