...
class FloatingViewState extends State<FloatingView>
  ...
  void initState() {
    ...
    _width = _floatingData.width.toDouble();
    _height = _floatingData.height.toDouble();
    isFullScreen = _floatingData.isInitFullscreen;
  }
  ...
  Widget build(BuildContext context) {
    ...
    pdebug("build======> $isFullScreen-$_top-$_left-$_width-$_height");
    ...
    return Stack(
      children: [
        AnimatedPositioned(
          ...
          left: _left,
          top: _top,
          width: isFullScreen ? _parentWidth : _width,
          height: isFullScreen ? _parentHeight : _height,
  ...
  _setScrollControl() {
    ...
    control.setScrollBottomRight(
      (bottom, right) => _scrollXY(
        _parentWidth - right - _width,
        _parentHeight - bottom - _height,
      ),
    );
  }
  ...
  _scrollXY(double x, double y) {
    if ((x > 0 || y > 0) && (_left != x || _top != y)) {
      ...
      _scrollController = AnimationController(
        duration: Duration(milliseconds: control.timeMillis),
        vsync: this,
      );
