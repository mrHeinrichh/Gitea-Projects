...
class _VoiceRecordButtonState extends State<VoiceRecordButton>
    ...
    if (widget.isRecording != oldWidget.isRecording) {
      if (!widget.isRecording) {
        ...
        _animationController.reverse().whenComplete(() async {
          ...
          if (recordTime.value >= 1000 && recordDecibels.value.length == 30) {
  ...
  List<double> getFilteredDecibelData() {
    int targetLength = 30;
    final step = recordDecibels.value.length ~/ targetLength;
    final List<double> tempDecibelValue = [];
    try {
      for (int i = 0; i < targetLength; i++) {
        final int start = i * step;
        final int end = (i + 1) * step < recordDecibels.value.length
            ? (i + 1) * step
            : recordDecibels.value.length;

        if (start < recordDecibels.value.length) {
