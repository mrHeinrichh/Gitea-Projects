...
import 'package:im_common/im_common.dart' as imcommon;
...
class RedPacketController extends GetxController
  ...
  final themedColor = imcommon.themeColor.obs;
  ...
  Future<void> confirmSend(String value) async {
    ...
    try {
      ...
      if (res.success()) {
        if (res.needTwoFactorAuthPhone || res.needTwoFactorAuthEmail) {
          ...
          if (resAgain.success()) {
            ...
            imcommon.showSuccessToast(resAgain.message.toString());
            Get.back();
          }
        } else {
          if (res.success()) {
      ...
      if (e.getPrefix() == ErrorCodeConstant.STATUS_MAX_TRY_PASSWORD_WRONG) {
        ...
        isPasscodeIncorrect.value = true;
        imcommon.showErrorToast(localized(incorrectPassword));
