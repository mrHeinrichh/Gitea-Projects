...
import 'package:dismissible_page/dismissible_page.dart' hide TransparentRoute;
...
import 'package:im_photo_view/photo_view_gallery.dart';
...
import 'package:jxim_client/im/media_detail/photo_view_event.dart';
...
class MediaDetailViewState extends State<MediaDetailView>
  ...
  final List<ToolOptionModel> _searchImageBottomOptionList = [
    _forwardOption,
    MessagePopupOption.delete.toolOption,
  ];
  ...
  final PhotoViewEvent photoViewEvent = PhotoViewEvent();
  ...
  void initState() {
    ...
    photoPageController = PageController(
    ...
    if (_isVideoAsset(widget.index)) {
      ...
      switch (widget.fromPage) {
        case FromPage.search:
          optionList.assignAll(_searchImageBottomOptionList);
          break;
        default:
  ...
  void onTap(String optionType, int index) async {
  ...
  void _onPhotoEdit() async {
    try {
      ...
      if (asset is AssetEntity) {
        ...
        filePath = downloadMgrV2.getLocalPath(fileName);

        // filePath ??= await downloadMgr.downloadFile(fileName);
        DownloadResult result = await downloadMgrV2.download(fileName);
  ...
  Future<void> confirmDelete(bool isAll) async {
    ...
    if (widget.fromPage == FromPage.search) {
      if (Get.isRegistered<ChatListController>()) {
        final chatListController = Get.find<ChatListController>();
        if (chatListController != null) {
          Chat? chat = objectMgr.chatMgr.getChatById(delMessage.chat_id);
          if (chat != null) {
            chatListController.deleteMessages(
              [delMessage],
              chat.chat_id,
              isAll: isAll,
            );
          ...
          children: <Widget>[
            ...
            Obx(() {
              return AnimatedPositioned(
                ...
                duration: const Duration(milliseconds: 200),
                child: Container(
                  width: subOptionList.isNotEmpty ? 220 : 0,
      ...
      () => RepaintBoundary(
        child: PhotoView.customChild(
          ...
          child: Image.file(
            editedFile.value,
            fit: BoxFit.contain,
            frameBuilder: onEditedPhotoViewLoadStateChanged,
          ),
        ),
      ),
    );
  ...
  Widget onEditedPhotoViewLoadStateChanged(_, child, frame, __) {
    if (frame != null) {
  ...
  Widget _getImageBottomBar(List<Widget> optionTab) {
    var forwardWidget = _forwardOption.isShow ? optionTab.first : null;
    if (forwardWidget != null) {
      optionTab.remove(optionTab.first);
    }
    return Row(
      mainAxisAlignment: _forwardOption.isShow
          ? MainAxisAlignment.spaceBetween
      ...
      children: [
        ...
        Row(
          children:
              List.generate(optionTab.length, (index) => optionTab[index]),
        ),
      ],
    );
  }
          ...
          AnimatedSize(
            ...
            child: Padding(
              ...
              child: TextField(
                ...
                cursorColor: colorBrightPrimary,
                style: const TextStyle(
                ...
                decoration: InputDecoration(
                  ...
                  isDense: true,
                  fillColor: colorBrightPlaceholder,
                  filled: true,
  ...
  Widget _buildMediaWidget(int index) {
    ...
    if (message.typ == messageTypeVideo || message.typ == messageTypeReel) {
      ...
      if (message.asset != null &&
        ...
        sourceExtension = path.extension(messageVideo.filePath).toLowerCase();
      }
  ...
  Widget buildAlbumDetailItemVideo(
    ...
    if (message.asset != null &&
      ...
      asset = media.url;
      sourceExtension = path.extension(media.filePath).toLowerCase();
    }
  ...
  Future<(String, String, String?, String?, int, int)> _getPreloadParams(
      int index) async {
  ...
  Widget buildName(Message message) {
    if (widget.fromPage == FromPage.search) {
      Map<String, String> chatNameMap = getChatNameMap(message);

      /// total width - (icon width * optionList.length) - (padding * 2)
      double maxWidth = MediaQuery.of(context).size.width -
          (24 * optionList.length) -
          (32 * 2);

      double width1 = maxWidth / 2;
      double width2 = maxWidth / 2;

      if ((chatNameMap['first']?.length ?? 0) <= 3) {
        width2 = maxWidth / 1.2;
      }

      if ((chatNameMap['second']?.length ?? 0) <= 3) {
        width1 = maxWidth / 1.2;
      }
      ...
      return Row(
        ...
        children: [
          ConstrainedBox(
            constraints: BoxConstraints(maxWidth: width1),
            child: Text(
              chatNameMap['first'] ?? '',
              style: TextStyle(
                color: colorBrightPrimary,
                ...
                height: im.ImLineHeight.getLineHeight(
                  fontSize: 17,
                  lineHeight: 23.8,
                ),
              ),
          ...
          ConstrainedBox(
            constraints: BoxConstraints(maxWidth: width2),
            child: Text(
              chatNameMap['second'] ?? '',
              style: TextStyle(
                color: colorBrightPrimary,
      ...
      if (message.send_id == 0) {
        ...
        child = NicknameText(
          ...
          isTappable: false,
          groupId: widget.contentController != null
