...
class _GeneralMediaPickerState extends State<GeneralMediaPicker> {
  ...
  void onAssetTap(BuildContext context, int index, AssetEntity asset) async {
    if (widget.provider.selectedAssets.isNotEmpty) {
      ...
      if (widget.typeRestrict) {
        ...
        if ((isDifferentType || (!allImage && isVideoConflict)) &&
            widget.provider.selectedAssets.first.id != asset.id) {
          imBottomToast(
            context,
            title: isDifferentType
                ? localized(momentRestrictSelectPhotoAndVideo)
    ...
    if (asset.type == AssetType.video &&
        widget.videoRestrictDuration != -1 &&
        asset.duration > widget.videoRestrictDuration) {
      imBottomToast(
        context,
        title: localized(momentRestrictSelectVideoDuration),
        icon: ImBottomNotifType.warning,
    ...
    Get.toNamed(
      ...
      if (notBlank(result)) {
        ...
        if (widget.videoRestrictDuration != -1 &&
            firstAsset.duration > widget.videoRestrictDuration) {
          imBottomToast(
            context,
            title: localized(momentRestrictSelectVideoDuration),
            icon: ImBottomNotifType.warning,
  ...
  void onAlbumTap() {
    Get.toNamed(
      ...
      final pathEntity = widget.provider.paths.first;
                ...
                Expanded(
                  child: AnimatedSwitcher(
                    ...
                    child: Text(
                      ...
                      titleSwap
                          ? localized(
                              widget.provider.selectedAssets.length > 1
                                  ? selectedAssetTexts
                                  : selectedAssetText,
                              params: [
