...
class AssetPreviewController extends GetxController
  ...
  void onInit() {
    ...
    if (arguments.containsKey('fromShortcut')) {
      fromShortcut = arguments['fromShortcut'];
    }
    ...
    if (provider != null && !isEdit.value && !showSelected) {
  ...
  void selectAsset(
    ...
    if (isEdit.value || provider == null) return;
    ...
    if (provider!.selectedAssets.contains(asset)) {
      ...
      if (pConfig != null && selectedAssetCount.value >= pConfig!.maxAssets) {
  ...
  void editAsset(BuildContext context) async {
    ...
    try {
      ...
      if (done) {
        ...
        // prevent new edit image will detect as new photo
        objectMgr.chatMgr.lastCheckedTime = DateTime.now();

        if (!editCompleter!.isCompleted) {
  ...
  Future<void> sendAsset() async {
    if (editCompleter != null && !editCompleter!.isCompleted) {
      editCompleter!.complete();
    }
