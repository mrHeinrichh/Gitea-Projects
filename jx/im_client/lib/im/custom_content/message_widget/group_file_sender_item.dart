...
class _GroupFileSenderItemState
    extends MessageWidgetMixin<GroupFileSenderItem> {
  ...
  bool get isDownloading => downloadProgress > 0;
  ...
  void initState() {
    ...
    fileDownloadService.on(
        FileDownloadService.fileDownloadComplete, _fileDownloadComplete);
  }
  ...
  void _fileDownloadComplete(sender, type, data) {
    final url = data['url'];
    final localPath = data['localPath'];
    if (url == widget.messageFile.url) {
  ...
  Future<void> msgOnTapped() async {
    ...
    if (isDownloading) {
      downloadProgress.value = 0.0;
      cacheMediaMgr.removeDownloadedMediaCache(widget.messageFile.url);
    ...
    return Obx(() {
      ...
      Widget fileTitle = Container(
        ...
        child: Text(
          ...
          style:
              jxTextStyle.headerText(color: themeColor).copyWith(height: 1.1),
          overflow: TextOverflow.ellipsis,
          maxLines: 2,
        ),
      );
      Widget fileSubtitle = Text(
        fileSize(widget.messageFile.size),
  ...
  Widget buildContent(Widget body, bool showPinned) {
    if (objectMgr.loginMgr.isDesktop) {
      return Expanded(
          child: Stack(
        children: <Widget>[
  ...
  Widget buildNormalFile(
    ...
    return ChatBubbleBody(
      ...
      body: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: <Widget>[
          if ((!isFirstMessage && !isPinnedOpen && widget.chat.isGroup) ||
              !widget.chat.isGroup)
          ...
          bottomWidget,
          SizedBox(height: emojiUserList.isNotEmpty ? 0 : 12.0),
