...
class TencentVideoStreamMgr {
  ...
  TencentVideoStream addController(TencentVideoConfig config, {int index = 0}) {
    ...
    if (!objectMgr.tencentVideoMgr.checkAllowPlay() && config.autoplay) {
  ...
  realignIndex(int indexToAlign, {int reducingOffset = 1}) {
    List<TencentVideoStream> itemsToReAlign =
        _data.where((element) => element.pageIndex > indexToAlign).toList(); //
  ...
  _onPlayerEventUpdate(int event, TencentVideoController controller) {
    ...
    switch (event) {
      ...
      case TXVodPlayEvent.PLAY_EVT_PLAY_PROGRESS:
        ...
        if (stream.isSeekingToEnd) return;
      ...
      case TXVodPlayEvent.PLAY_EVT_PLAY_LOADING:
        if (!stream.isRestarting && stream.showPause) {
    ...
    if (enteringBusinessPausePhase) {
      //若是业务所需还收到播放事件（就强制让播放器暂停）
  ...
  _onPlayerStateUpdate(
    ...
    if (state == TencentVideoState.SEEK_TO_END) {
      ...
      stream.isSeekingToEnd = true;
    }
    if (state == TencentVideoState.MUTE_LOADING) {
      state = stream.state.value;
      stream.showPause = false;
    }
