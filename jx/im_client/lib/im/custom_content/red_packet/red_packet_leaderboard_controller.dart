...
class RedPacketLeaderboardController extends GetxController
  ...
  RedPacketLeaderboardController();
  RedPacketLeaderboardController.desktop(dynamic args) {
    ...
    chat = arguments['chat'];
    if (arguments['toast_function'] != null) {
      desktopArgumentToast = arguments['toast_function']();
    }
  }
  ...
  String get isMeRedPacketAmountClaimed {
    for (User u in leaderBoardList) {
      if (objectMgr.userMgr.isMe(u.id)) {
        ReceiveInfo? info = redPacketData.value.receiveInfos
            .firstWhereOrNull((element) => element.userId == u.uid);
