...
class _ChatPopMenuSheetState extends State<ChatPopMenuSheet> {
  ...
  void initState() {
    ...
    if (isDesktop) {
      optionListFir.insertAll(optionListFir.length - 1, optionListSec);
  ...
  void onClick(String title) async {
    EasyDebounce.debounce(
      ...
      if (!(widget.message.isDisableMultiSelect)) {
      ...
      if (option.hasSubOption) {
        ...
        switch (title) {
          ...
          case 'copy': //复制
            if (objectMgr.loginMgr.isDesktop) {
              if (widget.message.typ == messageTypeImage ||
                ...
                DownloadResult r = await downloadMgrV2.download(imgObj.url);
                final result = r.localPath;
                // final result = await downloadMgr.downloadFile(imgObj.url);
                if (result != null) {
          ...
          case 'unpin':
            ...
            if (contentController.chatController.pinMessageList.isEmpty &&
        ...
        if (isDesktop &&
            (title != 'copy' && title != 'forward' && title != 'report')) {
          Get.back();
        }
      }
    });
  ...
  Future<void> saveMessageMedia(
    ...
    if (fileList.isEmpty) {
      ...
      if (isDesktop) {
        ...
        final dataFile = File(fileObj.path);
        ...
        String fileName =
            "HeyTalk Video ${formatTimestamp(DateTime.now().millisecondsSinceEpoch)}${path.extension(fileObj.path)}";
  ...
  int getMenuNum() {
    ...
    if (isSubList) {
      num = optionList.length + 1;
    }
    return num;
  }

  int getMenuEqualNum(int index) {
    if (shouldShowSeen || isSubList) {
      return index + 1;
    }
    return index;
  }
  ...
  Widget _buildDesktopMenuContainer() {
    return Material(
              ...
              if (isSubList) {
                if (index == 0) {
                  if (mounted) {
                    setState(() {
                      optionList =
                          isFirstMenuPage ? optionListFir : optionListSec;
                      isSubList = false;
                    });
                  }
                } else {
                      ...
                      ListView.builder(
                        ...
                        itemBuilder: (BuildContext context, int index) {
                          ...
                          final Widget childWidget = OverlayEffectMenu(
                            isHighLight: touchIndex == getMenuEqualNum(index),
                            child: Container(
                              height: 44,
