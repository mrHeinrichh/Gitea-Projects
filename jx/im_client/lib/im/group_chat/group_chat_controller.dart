...
class GroupChatController extends BaseChatController {
  ...
  void _setupGroup(Group group) {
    groupMembers.assignAll(group.members.cast<Map<String, dynamic>>());
    chatIsDeleted.value = !objectMgr.myGroupMgr.isGroupValid(chat.chat_id);
    permission.value = group.permission;
    groupMembers.assignAll(group.members.cast<Map<String, dynamic>>());
    isScreenshotEnabled =
        GroupPermissionMap.groupPermissionScreenshot.isAllow(group.permission);
    sharedDataManager.saveGroupInfo(group.toJson());
  }
