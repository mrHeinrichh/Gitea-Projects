...
class _MediaViewState extends MessageWidgetMixin<MediaView>
  ...
  loadMediaList() async {
    ...
    List<Map<String, dynamic>> tempList =
      ...
      'chat_id = ? AND chat_idx > ? AND chat_idx < ? AND (typ = ? OR typ = ? OR typ = ? OR typ = ?) AND (expire_time == 0 OR expire_time > ?) AND message_id != 0 AND ref_typ == 0',
      [
        widget.chat!.id,
