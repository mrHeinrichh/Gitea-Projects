...
class GroupChatInfoController extends GetxController
  ...
  Future<List> checkTaskExist() async {
    List<Map<String, dynamic>> tempList =
        await objectMgr.localDB.loadMessagesByWhereClause(
      'chat_id = ? AND chat_idx > ? AND chat_idx < ? AND typ = ? AND expire_time == 0 AND ref_typ == 0',
      [
        chat.value!.id,
        chat.value!.hide_chat_msg_idx,
        chat.value!.msg_idx + 1,
  ...
  void _onLastSeenChanged(Object sender, Object type, Object? data) {
    objectMgr.onlineMgr.friendOnlineTime.forEach((uid, lastOnline) {
      final existingIndex =
          groupMemberListData.indexWhere((user) => user.uid == uid);
      if (existingIndex != -1) {
        groupMemberListData[existingIndex].lastOnline = lastOnline;
      }
    });
  }
  ...
  processGroupCallInfo() {
    ...
    im.sharedDataManager.saveGroupInfo(group.toJson());
  }
    ...
    if (isOwner.value) {
      ...
      if (objectMgr.loginMgr.isDesktop) {
        ...
        Get.toNamed(RouteName.groupAddMember,
            arguments: {'group': group.value, 'memberList': userList}, id: 1);
      } else {
        Get.toNamed(
  ...
  void showMoreOptionPopup(BuildContext context) {
    ...
    List<ToolOptionModel> optionModelList = [
      ...
      ToolOptionModel(
        ...
        isShow: Config().e2eEncryptionEnabled && isOwner.value,
        tabBelonging: null,
        largeDivider: true,
