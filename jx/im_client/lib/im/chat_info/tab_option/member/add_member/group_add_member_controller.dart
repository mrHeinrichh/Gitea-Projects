...
class GroupAddMemberController extends GetxController {
  ...
  void onAddMember() async {
    ...
    try {
      if (chat?.isActiveChatKeyValid ?? false) {
        await objectMgr.encryptionMgr.startChatEncryption(
            selectedUser.map((element) => element.uid).toList(),
            chat: chat);
      }

      final invitedCount = selectedUser.length;
      String displayText;
      if (invitedCount <= 5) {
        displayText = '${localized(you)}${localized(chatInvite)}'+
            selectedUser
                .map((user) => objectMgr.userMgr.getUserTitle(user))
                .join(', ');
      } else {
        displayText = '${localized(you)}${localized(chatInvite)}$invitedCount ${localized(chatMemberJoinedThisGroup)}';
      }
