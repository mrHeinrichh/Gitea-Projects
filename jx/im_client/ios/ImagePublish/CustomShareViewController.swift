...
//        }
        ...
        if isChatDataEmpty {
            ...
            NSLog("Check container space: \(topSpace) | \(containerTop.constant)")
...
//                        if (data != nil) {
                        ...
                        let videoAsset = AVURLAsset(url: item as! URL, options: options)
                        guard let track = videoAsset.tracks(withMediaType: AVMediaType.video).first else { return }
...
extension CustomShareViewController: UICollectionViewDataSource, UICollectionViewDelegate, UICollectionViewDelegateFlowLayout{
    ...
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        ...
        if !chatDataArraySelected.isEmpty {
            let topSpace:CGFloat = view.safeAreaInsets.top + view.safeAreaInsets.bottom + self.cancelBtn.bounds.size.height + 8
            self.topSpace = topSpace
        } else {
            let topSpace:CGFloat = view.bounds.size.height - view.safeAreaInsets.top - view.safeAreaInsets.bottom - self.cancelBtn.bounds.size.height - 8  - 390 - 60
            self.topSpace = topSpace
        ...
        // Animate the layout change for a smooth effect
    ...
    func searchBar(_ searchBar: UISearchBar, textDidChange searchText: String) {
        ...
        if (!trimmedText.isEmpty){
            ...
            let array = self.chatDataArray.filter({ item in
                return item.name.lowercased().contains(trimmedText.lowercased())
            })
