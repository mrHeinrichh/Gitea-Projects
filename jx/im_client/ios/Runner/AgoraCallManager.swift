...
open class AgoraCallManager: NSObject, ObservableObject {
    ...
    var remoteFrameReady: Bool = false
    ...
    // 标记是否接听了通话
    var selfAccepted: Bool = false
    
    // 标记是否播放了来电铃声
    var playedRingSound: Bool = false
    ...
    @objc func appWillToForeground() {
        ...
        checkAndStopPipMode(checkTimes: 10)
    ...
    func checkAndStopPipMode(checkTimes: Int) {
    ...
    open func setupEngine(appId: String, isInviter: Bool, isVoiceCall:Bool, fps:Int, width:Int, height:Int) -> Void {
        ...
        if(isInviter && isVoiceCall){
            ...
            AudioUtils.shared.setAudioWhenRing()
        }
...
//        engine?.disableAudio()
        ...
        setLocalFirstFrameReady() //ios防止闪屏
    ...
    func playRingSound() {
        do {
            if (self.selfAccepted) {
                NSLog("[call] not do play ring sound, selfAccepted: true")
                return
            }
            
            if (self.playedRingSound) {
                NSLog("[call] not do play ring sound, playedRingSound: true")
                return
            }
            
            if (self.rtcChannelId == "") {
                NSLog("[call] not do play ring sound, rtcChannelId: %@", self.rtcChannelId)
                return
            }
            
            NSLog("[call] do play ring sound, %@", self.rtcChannelId)
            self.playedRingSound = true
            soundMgr.playRingSound(volume: 1.0)
        } catch {
           NSLog("[call] play ring sound err, \(error)")
        }
    }
    ...
    func releaseEngine(resetAudio: Bool) {
        ...
        self.pipController = nil
        self.isExit = false
        self.selfAccepted = false
        self.playedRingSound = false
        self.rtcChannelId = ""
        self.isCallkit = false
        ...
        self.destoryEngine()
        NSLog("[call] finish release engine")
    }
...
extension AgoraCallManager: AgoraRtcEngineDelegate {
    ...
    public func rtcEngine(_ engine: AgoraRtcEngineKit,
                   ...
                   elapsed: Int) {
        NSLog("remoteVideoStateChangedOfUid======> \(uid) - \(remoteUserId) | \(state) | \(reason)")
        if(uid == remoteUserId){
            if((state == .frozen && reason == .sdkInBackground) || (state == .stopped && reason == .remoteMuted)){
                self.remoteVideo.hasStream = false
                rtcChannel?.invokeMethod("iosCallInBackground", arguments: [
                    "isIosCallInBackground": true
                ])
            ...
            } else if(state == .starting || state == .decoding){
                self.remoteVideo.hasStream = true
                rtcChannel?.invokeMethod("iosCallInBackground", arguments: [
                    "isIosCallInBackground": false
                ])
            }
        }
    }
    ...
    private func setRemoteFirstFrameReady(){
        if(!remoteFrameReady){
            remoteFrameReady = true
            rtcChannel?.invokeMethod("onFirstRemoteVideoFrame",arguments: nil)
        }
    }
}
