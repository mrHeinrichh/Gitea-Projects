...
class AudioUtils: NSObject {
    ...
    func playSpeakerForVoice(){
        NSLog("[call] playSpeakerForVoice")
        do {
            let audioSession = AVAudioSession.sharedInstance()
    ...
    func setAudioWhenRing() {
        NSLog("[call] setAudioWhenRing")
        do {
            let audioSession = AVAudioSession.sharedInstance()
    ...
    func playEarpieceForVoice(){
        NSLog("[call] playEarpieceForVoice")
        do {
            let audioSession = AVAudioSession.sharedInstance()
            try audioSession.setCategory(.playAndRecord, mode: .default, options: [.allowBluetooth, .allowBluetoothA2DP])
            try audioSession.setActive(true)
    ...
    func routeToRingBackgorundMode(){
        NSLog("[call] routeToRingBackgorundMode")
        do {
            let audioSession = AVAudioSession.sharedInstance()
    ...
    func routeToRingSlientMode(){
        NSLog("[call] routeToRingSlientMode")
        do {
            let audioSession = AVAudioSession.sharedInstance()
            try audioSession.setCategory(.soloAmbient, mode: .default, options: [])
            try audioSession.setActive(true)
        } catch {
            print("[call] Failed to check background audio: \(error)")
            SentrySDK.capture(error: error)
        }
    }
