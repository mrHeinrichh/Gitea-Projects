...
//                setAudioToEarpiece()
            ...
            when (call.method) {
                ...
                "updateBadgeNumber" -> {
                    if (call.arguments is Int) {
                        ...
                        NotificationService.badgeNumber = number
                    }
                }
            }
        }
    ...
    private fun getCurBluetoothDevices(): MutableList<String> {
        ...
        try {
            ...
            val connectedDevices = devices.filter { it.type == AudioDeviceInfo.TYPE_BLUETOOTH_SCO || it.type == AudioDeviceInfo.TYPE_BLUETOOTH_A2DP  }.toSet()
            if (connectedDevices.isNotEmpty()) {
                list = connectedDevices.map { it.productName.toString() }.toMutableList()
            }
            return list
        } catch(e: Throwable) {
            Log.v("curBlueTooth", "curBlueTooth failed======> ${e.message}")
            return list
        }
    }
    ...
    fun setCommunicationDevice(targetDeviceType: Int) {
        audioManager.availableCommunicationDevices.firstOrNull { it.type == targetDeviceType }
            ?.let {
...
//        audioManager.isBluetoothScoOn = true
    ...
    private fun install24(file: File?) {
        ...
        if (Build.VERSION.SDK_INT < Build.VERSION_CODES.O) {
            ...
            val uri: Uri =
                FileProvider.getUriForFile(this, "${this.packageName}.fileProvider.install", file)
