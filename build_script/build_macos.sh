...
##处理一下输出文件
file_name="$macos_path/macos-release-v$version.app"
...
appName=`xcodebuild -showBuildSettings | grep PRODUCT_NAME | awk -F ' = ' '{print $2}'`
...
if [ $targetName == "Runner" ]; then
	flutter build macos --dart-define-from-file=profile.json --release
elif [ $targetName == "HeyTalk" ]; then
	flutter build macos --flavor $targetName --dart-define-from-file=release.json --release
else
	echo "Invalid Target"
	exit 1
fi
...
echo "Copy app"
cp -f $main_path/build/macos/build/Products/Release/$appName.app $file_name

xcrun altool --upload-app --type osx -f $file_name --username admin@jiangxia.com.sg --password fmxn-qtzt-nmxl-xowd
...
#	<key>provisioningProfiles</key>
#	<dict>
#    <key>com.jiangxia.im</key>
#    <string>jxim</string>
#    <key>com.jiangxia.im.ImagePublish</key>
#    <string>jximImagePublish</string>
#    <key>com.jiangxia.im.NotificationService</key>
