...
UIDeviceOrientation deviceOrientation = UIDeviceOrientationFaceUp;
...
- (void)handleMethodCallAsync:(FlutterMethodCall *)call
    ...
    if ([@"availableCameras" isEqualToString:call.method]) {
        ...
        if ([@"initialize" isEqualToString:call.method]) {
            ...
            [self sendDeviceOrientation:deviceOrientation];
            [_camera start];
            [self.camera setDeviceOrientation:UIDeviceOrientationFaceUp];
            [result sendSuccess];
            ...
            if (@available(iOS 10.0, *)) {
                ...
                dispatch_async(self.captureSessionQueue, ^{
                    // `FLTCam::setDeviceOrientation` must be called on capture session queue.
                    [self.camera setDeviceOrientation:deviceOrientation];
                    [self.camera captureToFile:result];
                    [self.camera setDeviceOrientation:UIDeviceOrientationFaceUp];
                });
            ...
            dispatch_async(self.captureSessionQueue, ^{
                // `FLTCam::setDeviceOrientation` must be called on capture session queue.
                [self.camera setDeviceOrientation:deviceOrientation];
                [self.camera startVideoRecordingWithResult:result];
                [self.camera setDeviceOrientation:UIDeviceOrientationFaceUp];
            });
            ...
            if(cameraId == -1){
                ...
                @try {
                  orientation = FLTGetUIDeviceOrientationForString(call.arguments[@"orientation"]);
                } @catch (NSError *e) {
                  [result sendError:e];
                  return;
                }
