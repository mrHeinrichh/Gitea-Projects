...
class CameraPickerState extends State<CameraPicker>
  ...
  void didChangeAppLifecycleState(AppLifecycleState state) {
    ...
    if (state == AppLifecycleState.inactive) {
      isSwitchFrontDesk = false;
      c.dispose();
    } else if (state == AppLifecycleState.resumed) {
      isSwitchFrontDesk = true;
    ...
    ambiguate(WidgetsBinding.instance)?.addPostFrameCallback((_) async {
      ...
      if (cameraDescription == null) {
        ...
        if (cameras.length > 2) {
          ...
          for (final CameraDescription item in cameras) {
      ...
      try {
          ...
          <Future<void>>[
            if (pickerConfig.lockCaptureOrientation != null)
              newController.lockCaptureOrientation(
                  orientation: pickerConfig.lockCaptureOrientation),
            newController
        ...
        newController.setFlashMode(FlashMode.off);
  ...
  Future<void> setDeviceOrientation(DeviceOrientation orientation) async {
    ...
    await controller.lockCaptureOrientation(
        cameraId: -1, orientation: orientation);
  }
  ...
  Future<void> takePicture() async {
    ...
    try {
      ...
      final XFile oldFile = await controller.takePicture();
      // 前置摄像头镜像一下
      if (currentCamera.lensDirection == CameraLensDirection.front) {
        final Uint8List imageBytes = await oldFile.readAsBytes();
        final img.Image? originalImage = img.decodeImage(imageBytes);
        final img.Image fixedImage = img.flipHorizontal(originalImage!);
        final File file = File(oldFile.path);
      ...
      final AssetEntity? entity = await generateAssetEntityAndPop(
        file: oldFile,
  ...
  Future<AssetEntity?> generateAssetEntityAndPop({
    required XFile file,
  ...
  Widget buildCameraSwitch(BuildContext context) {
    return NativeDeviceOrientedWidget(
      landscape: (BuildContext context) {
        return _switchCamerasButton(DeviceOrientation.landscapeLeft);
      },
      landscapeLeft: (BuildContext context) {
        return _switchCamerasButton(DeviceOrientation.landscapeLeft);
      },
      landscapeRight: (BuildContext context) {
        return _switchCamerasButton(DeviceOrientation.landscapeRight);
      },
      portrait: (BuildContext context) {
        return _switchCamerasButton(DeviceOrientation.portraitUp);
      },
      portraitUp: (BuildContext context) {
        return _switchCamerasButton(DeviceOrientation.portraitUp);
      },
      portraitDown: (BuildContext context) {
        return _switchCamerasButton(DeviceOrientation.portraitDown);
      },
  ...
  Widget _switchCamerasButton(DeviceOrientation orientation) {
    int v = 0;
    switch (orientation) {
          ...
          Expanded(
            child: buildInitializeWrapper(
              builder: (CameraValue v, __) {
                ...
                return buildCameraSwitch(context);
              },
            ),
          ),
        ],
  ...
  Widget buildCameraPreview({
    ...
    Widget preview = Listener(
      ...
      child: GestureDetector(
        ...
        child: (innerController != null && isSwitchFrontDesk)
            ? _CameraPreview(controller, orientation)
...
class _CameraPreview extends StatelessWidget {
  ...
  const _CameraPreview(this.controller, this.orientation,
      {Key? key, this.child})
      : super(key: key);
  ...
  Widget _wrapInRotatedBox({required Widget child}) {
    ...
    //   return child;
    // }
    final RotatedBox child0 = RotatedBox(
  ...
  DeviceOrientation _getApplicableOrientation() {
    ...
    if (controller.value.isRecordingVideo) {
      ori = orientation;
      if (ori == DeviceOrientation.landscapeRight) {
        ori = DeviceOrientation.landscapeLeft;
      } else if (ori == DeviceOrientation.landscapeLeft) {
      ...
      ori = controller.value.previewPauseOrientation ??
