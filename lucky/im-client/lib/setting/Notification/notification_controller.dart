...
class NotificationController extends GetxController {
  ...
  void initNotificationSetting() async {
    ...
    setNotificationTypeLocalStorage(
        NotificationSection.privateChat, data.privateChatNotificationType);
    setNotificationTypeLocalStorage(
        NotificationSection.groupChat, data.groupChatNotificationType);
    setNotificationTypeLocalStorage(
        NotificationSection.wallet, data.walletNotificationType);
    setNotificationTypeLocalStorage(
        NotificationSection.friend, data.friendNotificationType);
  ...
  void setPreview(bool value) {
    ...
    if (notificationSection.value != NotificationSection.none) {
      _settingServices.setPreviewNotification(
          notificationSection.value, value ? 1 : 0);
    }
  }
  ...
  Future<void> changeNotificationMode(NotificationMode notificationMode) async {
    final result = await _settingServices.setGlobalNotification(
        notificationSection.value, notificationMode);
  ...
  void showNotificationTypePopUp(BuildContext context) {
    notificationTypeList.forEach((item) {
      item.isSelected = false;
      if (item.value == getNotificationMode().value) {
    ...
    Toast.showBottomSheet(
      ...
      container: CustomBottomSheet(
        ...
        selectionOptionModelList: notificationTypeList,
        callback: (int index) {
          changeNotificationMode(notificationTypeList[index].value!.toMode);
          Get.back();
        },
      ),
    );
  ...
  Future<void> setNotificationTypeLocalStorage(
      ...
      NotificationMode notificationMode) async {
  ...
  NotificationMode getNotificationTypeLocalStorage(
      NotificationSection notificationSection, bool notificationStatus) {
    String notificationSectionKey = "";
    switch (notificationSection) {
      case NotificationSection.privateChat:
        notificationSectionKey = LocalStorageMgr.PRIVATE_CHAT_NOTIFICATION;
    ...
    if (notificationStatus) {
      ...
      if (notificationMode != null) {
        return notificationMode.toMode;
      } else {
  ...
  Future<void> setMessageSoundStatusRemote(bool status) async {
    final bool res = await updateStore(
        StoreData.messageSoundData.key, status.toString(),
        isBroadcast: true);
    if (res) {
...
extension notificationIntUtils on int {
  NotificationMode get toMode {
    switch (this) {
      case 0:
        return NotificationMode.soundVibrate;
      case 1:
