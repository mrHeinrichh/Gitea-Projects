...
abstract class AutoScrollController implements ScrollController {
  ...
  Future scrollToIndex(int index,
      ...
      AutoScrollPosition? preferPosition, double extraOffset = 0});
...
mixin AutoScrollControllerMixin on ScrollController
  ...
  Future scrollToIndex(int index,
      {Duration duration = scrollAnimationDuration,
      AutoScrollPosition? preferPosition, double extraOffset = 0}) async {
    return co(
        this,
        () => _scrollToIndex(index,
            duration: duration, preferPosition: preferPosition, extraOffset: extraOffset));
  }
  ...
  Future _scrollToIndex(int index,
    ...
    Future makeSureStateIsReady() async {
      for (var count = 0; count < maxBound; count++) {
        if (_isEmptyStates) {
          await _waitForWidgetStateBuild();assert(isAnim);
        } else
          return null;
      }
    ...
    if (isIndexStateInLayoutRange(index)) {
      ...
      await _bringIntoViewportIfNeed(index, preferPosition,
          (double offset) async {
            isAnim ? await animateTo(offset, duration: duration, curve: Curves.ease) : jumpTo(offset + extraOffset);
      ...
      final moveDuration = isAnim ? duration ~/ defaultDurationUnit : Duration.zero;
