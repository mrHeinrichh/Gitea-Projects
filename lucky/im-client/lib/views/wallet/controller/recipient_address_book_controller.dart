...
class RecipientAddressBookController extends GetxController
  ...
  void getRecipientAddressList(
      {required String currencyType, String? netType}) async {
    final data = await walletServices.getRecipientsAddress(
        currencyType: currencyType, netType: netType);
  ...
  Future<void> deleteRecipientAddress(List<AddressModel> addrList) async {
    try {
      await Future.forEach(addrList, (addr) async {
        final result = await walletServices.deleteRecipientAddress(addrID: addr.addrID);
        if (result) {
          filterRecipientAddressList.removeWhere((element) => element.addrID == addr.addrID);
          allRecipientList.removeWhere((element) => element.addrID == addr.addrID);
        }
      });
      //确保所有请求完成之后再刷新钱包列表
  ...
  void onTapAddress(AddressModel model) async {
    if (isMultiSelect.value) {
      ...
      final addressModel =
          await Get.toNamed(RouteName.addAddressView, arguments: {
        'edit': true,
        'address': model,
      });
