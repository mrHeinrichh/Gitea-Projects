...
class ExtentMyTextSelectionControls extends TextSelectionControls {
  ...
  Future<void> handlePaste(TextSelectionDelegate delegate) async {
    var _imgData = ExtentMyTextSelectionControls.imgPasteData;
    if (_imgData != null) {
      if (_imgData is Uint8List && _imgData.length > 0) {
        if (pasteImgIndex == -1) {
          String? idxStr =
        ...
        String _path = downloadMgr.appDocumentRootPath +
            "/cache_pasted${pasteImgIndex}.jpg";
        pasteImgIndex++;
        ...
        File vFileSave = File(_path);
        if (!vFileSave.existsSync()) {
          vFileSave.createSync(recursive: true);
        }
        var ff = await vFileSave.writeAsBytes(_imgData, flush: true);
        if (ff.existsSync()) {
  ...
  Widget buildHandle(
    ...
    final Widget customPaint = CustomPaint(
      painter: _TextSelectionHandlePainter(colorE5454D),
    );
