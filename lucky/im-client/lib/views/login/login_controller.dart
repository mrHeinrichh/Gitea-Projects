...
import 'components/term_service_bottom_sheet.dart';
...
class LoginController extends GetxController with GetTickerProviderStateMixin {
  ...
  Future<void> successVerification() async {
    phoneError.value = "";
    emailError.value = "";
    ...
    try {
      ...
      if (selectMode.value == PHONE_NUMBER) {
        res = await getOTP(objectMgr.loginMgr.mobile,
            objectMgr.loginMgr.countryCode, OtpPageType.login.type);
      } else if (selectMode.value == EMAIL_ADDRESS) {
        res = await getOTPByEmail(
            objectMgr.loginMgr.emailAddress, OtpPageType.login.type);
      }
      if (res) {
        //成功获取验证码才去到另一页
        ...
        Get.toNamed(
          RouteName.otpView,
          arguments: {'from_view': OtpPageType.login.page},
        );
        if (selectMode.value == PHONE_NUMBER) {
      ...
      if (e is AppException) {
        if (e.getPrefix() == ErrorCodeConstant.STATUS_OTP_REACH_LIMIT) {
          Toast.showToastMessage(localized(homeOtpManyTimes));
        } else if (e.getPrefix() == CodeDefine.USER_MOBILE_ERROR) {
          phoneError.value = localized(invalidPhoneNumber);
        } else {
          String errorMessage = e.getMessage();
          if (e is NetworkException) {
            errorMessage = localized(noNetworkCheckYourEmailSettingsAndTryAgain);
          }
          if (selectMode.value == PHONE_NUMBER) {
  ...
  Future<void> checkPhoneNumber(String phoneNumber) async {
    ...
    if (contactNumber.length > 1) {
      if (objectMgr.loginMgr.isDesktop) {
        ...
        final validMobile = frPhone1.isValid(type: DPN.PhoneNumberType.mobile);
  ...
  void getUpdatedDropdown() async {
    ...
    index = countryCodeList.indexWhere((element) =>
  ...
  void showTermService() {
    showModalBottomSheet(
      ...
      builder: (BuildContext context) {
        return TermServiceBottomSheet(agreeCallback: () {
