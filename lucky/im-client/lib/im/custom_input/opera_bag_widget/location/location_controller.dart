...
class LocationController extends GetxController {
  static late LatLng cachedLocation = const LatLng(0, 0);
  static late List<Map<String, dynamic>> cachedNearbyLocations = [];
  ...
  final currentLocation = LatLng(1.28000, 103.85000).obs;
  ...
  Future<void> getPermissionAndLocation() async {
    try {
      ...
      if (!isSameLocation || cachedNearbyLocations.length == 0) {
  ...
  Future<void> moveCameraToTargetPosition(
    ...
    return mapController.moveCamera(CameraUpdate.newCameraPosition(
      CameraPosition(
  ...
  Future<void> getNearbyLocations() async {
    try {
      ...
      print('Location: ${nearbyLocations.value}');
      if (nearbyLocations.value.isNotEmpty) {
        cachedNearbyLocations = nearbyLocations.value;
      } else {
