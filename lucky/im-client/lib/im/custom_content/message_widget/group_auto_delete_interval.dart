  ...
  Widget build(BuildContext context) {
    ...
    return Align(
      ...
      child: Container(
          ...
          child: Text(
            "${objectMgr.userMgr.isMe(widget.messageInterval.owner) ? localized(you) : objectMgr.userMgr.getUserTitle(objectMgr.userMgr.getUserById(widget.messageInterval.owner))}$intervalContent",
            style: jxTextStyle.textStyle12(color: JXColors.primaryTextWhite),
    ...
    if (widget.messageInterval.interval == 0) {
      ...
      return ' ${localized(turnOnAutoDeleteMessage, params: [
            "${localized(isSingular ? secondParam : secondsParam, params: [
                  "${widget.messageInterval.interval}"
                ])}"
          ])}';
      ...
      return ' ${localized(turnOnAutoDeleteMessage, params: [
            "${localized(isSingular ? minuteParam : minutesParam, params: [
                  "${widget.messageInterval.interval ~/ 60}"
                ])}"
          ])}';
      ...
      return ' ${localized(turnOnAutoDeleteMessage, params: [
            "${localized(isSingular ? hourParam : hoursParam, params: [
                  "${widget.messageInterval.interval ~/ 3600}"
                ])}"
          ])}';
      ...
      return ' ${localized(turnOnAutoDeleteMessage, params: [
            "${localized(isSingular ? dayParam : daysParam, params: [
                  "${widget.messageInterval.interval ~/ 86400}"
                ])}"
          ])}';
    } else {
      ...
      return ' ${localized(turnOnAutoDeleteMessage, params: [
            "${localized(isSingular ? monthParam : monthsParam, params: [
                  "${widget.messageInterval.interval ~/ 2592000}"
                ])}"
          ])}';
    }
  }
