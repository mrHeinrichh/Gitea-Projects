...
class ChatListController extends GetxController {
  ...
  void onInit() async {
    ...
    chatListEvent.on(
        ChatListEvent.eventSearchStateChange, _onSearchStateChanged);
  ...
  void _onChatDeleted(_, __, Object? deletedChat) {
      ...
      if (!deletedChat.isVisible) {
        ...
        if (removeIdx != -1) {
          ...
          Get.findAndDelete<ChatItemController>(
              tag: 'chat_item_${deletedChat.id.toString()}', force: true);
        }
      } else {
        loadChatList();
      }
  ...
  void onItemClick(int index) {
    Chat _chat = chatList[index];
    Routes.toChat(chat: _chat);
  }
        ...
        return CustomConfirmationPopup(
          ...
          confirmCallback: () {
            ...
            ImBottomToast(
              ...
              duration: 5,
              timerFunction: () {
                selectedChatIDForEdit.forEach((chat_id) {
                  Chat? chat = chatList.firstWhereOrNull(
                      (element) => element.chat_id == chat_id);
                  if (chat != null) {
    ...
    showModalBottomSheet(
      ...
      builder: (BuildContext context) {
        return CustomConfirmationPopup(
          ...
          confirmButtonColor: JXColors.red,
          cancelButtonColor: accentColor,
          confirmButtonText: localized(deleteParamChat,
              params: ['${selectedChatIDForEdit.length}']),
          cancelButtonText: localized(buttonCancel),
          confirmCallback: () {
            isSelectMore.value = false;
            ...
            ImBottomToast(
              context,
              title: localized(deleteParamChat,
                  params: ['${selectedChatIDForEdit.length}']),
              icon: ImBottomNotifType.timer,
              duration: 5,
              withCancel: true,
  ...
  Future<void> scanQRCode({
    Function(String)? didGetText = null,
    bool isWallet = false,
  }) async {
  ...
  void onChatVisible(Chat chat) =>
      objectMgr.scheduleMgr.chatMessageTask.addChatItem(chat);
  ...
  void tapForEdit(int chat_id, int typ) {
    if (typ == chatTypeSaved) {
      Toast.showToast(localized(hideOrDeleteSavedMessageIsNotAllowed),
          isStickBottom: false);
    } else {
      selectedChatIDForEdit.contains(chat_id)
          ? selectedChatIDForEdit.remove(chat_id)
          : selectedChatIDForEdit.add(chat_id);
      update();
    }
  }
