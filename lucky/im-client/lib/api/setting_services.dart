...
class SettingServices {
  Future<Secure?> getPasscodeSetting() async {
    try {
      final ResponseData res = await Request.doGet('/app/api/wallet/settings');
  ...
  Future<Map<String, dynamic>> getPrivacySetting() async {
    try {
      final ResponseData res =
          await Request.doGet('/app/api/account/settings/privacy/get');
  ...
  Future<bool> setGlobalNotification(NotificationSection notificationSection,
  ...
  Future<bool> setPreviewNotification(
      NotificationSection notificationSection, int previewMode) async {
    final Map<String, dynamic> dataBody = {
      'preview': previewMode,
      'type': notificationSection.value,
    };
  ...
  Future<List<int>> setAllExceptionNotification(List<int> chatIDs,
      {int expiry = 0}) async {
    final Map<String, dynamic> dataBody = {
      'chat_ids': chatIDs,
      'expiry': expiry,
...
class PermissionSetting {
  ...
  PermissionSetting(
      {this.sendMediaPermission = true, this.sendStickerPermission = true});
...
class NotificationSetting {
  ...
  static NotificationSetting fromJson(dynamic data) {
    return NotificationSetting(
      ...
      privateChatNotificationType: (data['private_chat_mute'] as int).toMode,
      groupChatNotificationType: (data['group_chat_mute'] as int).toMode,
      walletNotificationType: (data['wallet_mute'] as int).toMode,
      friendNotificationType: (data['friend_mute'] as int).toMode,
...
class Version {
  ...
  Future<PlatformDetail> getAppVersionInfo() async {
    ...
    if (Platform.isAndroid) {
      ...
      platform = DownloadPlatform.play.value;
    } else if (Platform.isIOS) {
      os = OsType.ios.value;
      platform = Config().isTestFlight ? DownloadPlatform.testflight.value : DownloadPlatform.supersign.value;
