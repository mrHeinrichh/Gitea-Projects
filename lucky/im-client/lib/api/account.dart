...
Future<Account> accountLogin() async {
  ...
  dataBody["device_id"] = await objectMgr.loginMgr.deviceId;
...
Future<User> updatePhone(
    String countryCode, String contactNumber, String vCode) async {
  Map<String, dynamic> dataBody = {};
  dataBody["country_code"] = countryCode;
...
Future<User> updateUserDetail(Map<String, dynamic> data) async {
  try {
    final ResponseData res =
        await Request.doPost("/app/api/account/update", data: data);
...
Future heartbeat(int cur_time, {isOnline = true}) async {
  ...
  final res = await Request.doGet("/app/api/account/heartbeat", data: data);
  if (res.success()) {
    objectMgr
...
Future<bool> userLogout() async {
  try {
    final res = await Request.doGet("/app/api/auth/logout");
    if (res.success()) {
      return true;
    } else {
      throw AppException(res.code, res.message);
...
Future<CheckOtpModel?> checkOtpCode(String contact, String countryCode,
    String email, int type, String code) async {
  Map<String, dynamic> dataBody = {};
...
Future<ResponseData> updateVoipSession(int udid, int status,
...
Future<List> getTranslation() async {
  try {
    final ResponseData res = await Request.doGet(
      '/app/api/version/translation',
      needToken: false,
    );
...
Future<TranscribeModel> getTranscribeText(
    String languageTo, String mediaPath) async {
  final Map<String, dynamic> dataBody = {
    'lang_to': languageTo,
...
Future<bool> feedback(
    String category, String description, List<String> attachments) async {
  Map<String, dynamic> dataBody = {};
  ...
  try {
    final ResponseData res =
        await Request.doPost("/app/api/account/report-issue", data: dataBody);
...
Future<bool> updateStore(String key, String value,
