...
import '../im/custom_content/message_widget/album_cell.dart';
...
///  实际根据图片的宽高来采用不同的布局
class NewAlbumUtil {
  static getMaxHeight(int length, double maxWidthRatio) {
    switch (length) {
      case 1:
      case 2:
        return ((ObjectMgr.screenMQ!.size.width *
                    (objectMgr.loginMgr.isDesktop ? 0.25 : 1)) *
                (294 / _height) *
                maxWidthRatio)
            .toDouble();
      case 3:
      ...
      case 10:
        return ((ObjectMgr.screenMQ!.size.width *
                    (objectMgr.loginMgr.isDesktop ? 0.250 : 1)) *
                (448 / _height) *
                maxWidthRatio)
            .toDouble();
    }
  ...
  static getMaxWidth(int length, double maxWidthRatio) {
    ...
    return (ObjectMgr.screenMQ!.size.width * (300 / _width) * maxWidthRatio)
        .toDouble();
  }

  static double getCellHeight(CellSize cellSize) {
    return (ObjectMgr.screenMQ!.size.width *
            (objectMgr.loginMgr.isDesktop ? 0.248 : 1)) *
        cellSize.height;
  }

  static double getCellWidth(CellSize cellSize) {
    return (ObjectMgr.screenMQ!.size.width *
            (objectMgr.loginMgr.isDesktop ? 0.6 : 1)) *
        cellSize.width;
  }
  ...
  static List<Widget> buildRow(
    ...
    if (items.length == 2) {
      return buildRow2(items, isBorderRadius, isSender, isForwardMessage,
          isDesktop, onShowAlbum, maxWidthRatio, assetMessage);
    }
  ...
  static List<Widget> buildColumn(
    ...
    switch (length) {
      case 1:
        return buildColumn1(items, isDesktop, isSender, isForwardMessage,
            isBorderRadius, onShowAlbum, maxWidthRatio, assetMessage);
      case 3:
        return buildColumn3(
          items,
          isDesktop,
    ...
    if (items.length % 2 == 0) {
      return buildColumnEven(items, isDesktop, maxWidthRatio, isBorderRadius,
          isSender, isForwardMessage, onShowAlbum, assetMessage);
    } else {
  ...
  static Widget buildContent(int index,
      {required bool isDesktop,
      bool isSender = false,
      bool isForwardMessage = false,
      bool isBorderRadius = false,
  ...
  static List<Widget> addRow({
    ...
    for (int i = 0; i < data.length; i++) {
      ...
      if (i != data.length - 1) {
        list.add(const SizedBox(width: 1));
      }
    }
    return list;
  }
...
const double _cellHeight8_2 = 125 / _height;
...
const double _cellWidth2 = 158 / _width;
const double _cellWidth3_1 = 304 / _width;
const double _cellWidth3_2 = 153 / _width;
const double _cellWidth4 = 158 / _width;
const double _cellWidth5_1 = 153 / _width;
const double _cellWidth5_2 = 102 / _width;
const double _cellWidth6 = 158 / _width;
const double _cellWidth7_1 = 153 / _width;
const double _cellWidth7_2 = 102 / _width;
const double _cellWidth8_1 = 153 / _width;
