...
class ClientAction {
  ...
  ClientAction.created(this.action, this.request_id, this.data, this.code, this.message);
}
  ...
  UpdateBlockParser.created(String content) {
    ...
    for (var tableName in data.keys) {
      if (tableName == cmdTopic) {
        ...
        List rows = obj['r'];
        for (var row in rows) {
          clientActions.add(ClientAction.created(row['action'], row['request_Id'], row['data'], row['code'], row['message']));
        }
        ...
        for (var item in table) {
          ...
          httpUpdateBlockBeans.add(HttpUpdateBlockBean.created(
              item['seq'], item['status'], jsonDecode(body)));
        }
        ...
        for (var opt in table.keys) {
          ...
          for (var item in rows) {
            // 是否需要合并
            updateOprateBeans.add(SysOprate.created(
                item['type'], item['sub_type'], item['data']));
          }
        }
        ...
        for (var opt in table.keys) {
          List rows = table[opt];
          for (var item in rows) {
            updateChatReadBeans.add(UpdateBlockBean.created(
                opt, tableName, opt == blockOptReplace ? [item] : item));
          }
        }
        ...
        for (var opt in table.keys) {
          List rows = table[opt];
          for (var item in rows) {
            updateChatDeleteBeans.add(UpdateBlockBean.created(
                opt, tableName, opt == blockOptDelete ? [item] : item));
          }
        }
        ...
        for (var opt in table.keys) {
          List rows = table[opt];
          for (var item in rows) {
            messageHistoryBeans.add(UpdateBlockBean.created(
                opt, DBMessage.tableName, opt == blockOptReplace ? [item] : item));
          }
        }
        ...
        for (var opt in table.keys) {
          ...
          for (var item in rows) {
            item = parsePbMessage(item);
            updateBlockBeans.add(UpdateBlockBean.created(
                opt, tableName, opt == blockOptReplace ? [item] : item));
          }
        }
      } else if(tableName != "message"){ //实时消息改为 message_realtime_pb字段，之后message字段的内容将会被删除， 删除后改为else即可
        // 对象更新
        Map<String, dynamic> table = data[tableName];
        for (var opt in table.keys) {
          List rows = table[opt];
