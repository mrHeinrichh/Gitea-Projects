  ...
  List<T> getList<T extends RowObject>({int? count}) {
    ...
    if (count != null) {
      if (_list.length > count) {
        _list.removeRange(count, _list.length);
      }
    }
    return _list;
  }
...
class SharedRemoteDB extends EventDispatcher {
  ...
  /// 所有的数据表格
  static Map<String, SharedTable> _tables = {};
  ...
  /// 注册模型类
  SharedTable registerModel<T extends Poolable>(String tableName,
  ...
  Future<int?> applyUpdateBlock(UpdateBlockBean block,
      {bool save = true, bool notify = true, bool vibrate = false}) async {
  ...
  applyBatchRows(int uid, UpdateBlockBean block, int dbRetryTimes, Function? fun) {
    ...
    Isolate.spawn(applyBatchReplaceBlock,
        [uid, block, receivePort.sendPort, rootIsolateToken, dbRetryTimes]);
  }

  static Future<void> applyBatchReplaceBlock(List<Object> args) async {
    bool success = false;

    final uid = args[0];
    final block = args[1] as UpdateBlockBean;
    final sendPort = args[2] as SendPort;
    final rootIsolateToken = args[3] as RootIsolateToken;
    final dbRetryTimes = args[4] as int;
    ...
    var table = _getTable(blockCtl);
    if (table == null) {
      JsonObjectPool pool = JsonObjectPool(RowObject.creator);
      table = SharedTable(blockCtl, pool);
      setTable(table);
    }

    List<Map<String, dynamic>> addObjs = [];
    List<Map<String, dynamic>> updateObjs = [];
    ...
    for (int i = 0; i < allRow.length; i++) {
      ...
      if (obj == null) {
        ...
        if (obj != null) {
          obj.init(it);
          await table.addRow(obj, false);
          final data = jsonDecode(jsonEncode(obj));
          addObjs.add(data);
        }
      } else {
        await table.modifyRow(it, false);
        final data = jsonDecode(jsonEncode(obj));
        updateObjs.add(data);
      }
    }
    ...
    for (int i = 0; i < dbRetryTimes && !success; i++) {
      try {
        await _db.transaction((txn) async {
          for (final values in addObjs) {
            var id = values['id'];
            if (id != null) {
              txn.insert(table.tableName, values,
                  conflictAlgorithm: ConflictAlgorithm.ignore);
            }
          }

          for (final values in updateObjs) {
            var id = values['id'];
            if (id != null) {
              txn.update(table.tableName, values,
                  where: "id = ?",
                  whereArgs: [id],
                  conflictAlgorithm: ConflictAlgorithm.ignore);
            }
          ...
          try {
            ...
            pdebug("SUCESS ID : COMMIT ");
            success = true;
          } catch (e) {
            pdebug("failed to save to commit BRECASE ==> ${e.toString()}");
            success = false;
          }
        });
      } catch (e) {
        pdebug("DatabaseException ==> ${e.toString()}");
        success = false;
      }
    }
  ...
  Future<int?> _addRow(SharedTable table, dynamic id, dynamic data,
      {bool save = true, bool notify = true}) async {
    int? count = 0;
    RowObject? obj = table._getRow(id);
    if (obj == null) {
      obj = table._pool.fetch();
  ...
  void _delRow(SharedTable table, dynamic id,
      {bool save = true, bool notify = true}) {
  ...
  Future<int?> _updateRow(SharedTable table, dynamic data,
      {bool save = true, bool notify = true}) async {
    int? count = 0;
  ...
  Future<int?> _replaceRow(SharedTable table, RowObject row, dynamic data,
      {bool save = true, bool notify = true}) async {
    int? count = 0;
    count =  await table.modifyRow(data, save);
    if (count != null && notify) {
  ...
  /// 取得符合条件记录
  T? findOne<T>(String tableName, Map<String, dynamic> wheres,
      {FilterFunc? f}) {
    var tbl = getTable(tableName);
    if (tbl == null) return null;
  ...
  /// 取得符合条件记录
  List<RowObject>? findAll(String tableName, Map<String, dynamic> wheres,
      {FilterFunc? f}) {
