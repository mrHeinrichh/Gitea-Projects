...
import 'package:jxim_client/utils/timer_mgr.dart';

final dunMgr = DunMgr();

class DunMgr extends OnUpdateActor {
  ///当前token
  static String _currentToken = "token";

  Future<int> initAsync(String key) async {
    assert(key.isNotEmpty);
    int begin = DateTime.now().millisecondsSinceEpoch;
    mypdebug("YunCeng.initAsync() begin");
    int code = await FlutterYunCengKiwi.initAsync(key, _currentToken);
    int end = DateTime.now().millisecondsSinceEpoch;
    mypdebug("YunCeng.initAsync() => $code, cost ${end - begin} ms");
    return code;
  }
  ...
  Future<Uri?> serverToLocal(Uri uri) async {
    dynamic result = await FlutterYunCengKiwi.getProxyTcpByDomain(_currentToken, uri.host, "");
    int code = result["code"] is int ? result["code"] : int.parse(result["code"]);
    pdebug("Kiwi serverToLocal ${uri.host} return $code");
    
    if (code == 0) {
      return uri.replace(port: int.parse(result["target_port"]), host: result["target_ip"]);
    }
    return null;
  }
  ...
  Future<int> restart() async {
    ...
    mypdebug("YunCeng.restartAllServer() => $code");
    return code;
  }
