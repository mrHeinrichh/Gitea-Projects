...
abstract class AssetPickerProvider<Asset, Path> extends ChangeNotifier {
  ...
  Future<void> selectAsset(Asset item) async {
    ...
    item as AssetEntity;
    // final File? data = await item.originFile;
    // if (data != null && data.lengthSync() > pow(2, 30)) {
    //   // Fluttertoast.showToast(msg: 'Selected media cannot exceed 1 GB')
    //   BotToast.showText(
    //     duration: const Duration(milliseconds: 1600),
    //     text: 'Selected media cannot exceed 1 GB',
...
class DefaultAssetPickerProvider
  ...
  Future<void> getPaths() async {
    ...
    for (final AssetPathEntity pathEntity in list) {
      ...
      if (removeLivePhotos && pathEntity.darwinSubtype == PMDarwinAssetCollectionSubtype.smartAlbumLivePhotos) {
        continue;
      }
  ...
  Future<void> getAssetsFromPath(int page, AssetPathEntity path) async {
    List<AssetEntity> list = await path.getAssetListPaged(
      page: page,
      size: pageSize,
    );
    if (path.type == RequestType.video && removeLivePhotos) {
      list.removeWhere((element) => element.type == AssetType.image);
    }
